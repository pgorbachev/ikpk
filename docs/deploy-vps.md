...
1. **`\$` → `$`.** На голове ветки реализации таких вхождений **десять** на шести строках
   (`grep -c '\\\$' scripts/bootstrap-vps.sh`). Два из них — конец строки в регулярном
   выражении (`location ~ \.(pf_fragment|pf_index|pf_meta)\$`,
   `location ~ ^/sitemap.*\.xml\$`), и **именно они опасны**: в PCRE `\$` означает
   *литеральный символ доллара*, а не конец строки, поэтому перенесённое как есть правило не
   совпадёт ни с одним реальным адресом. `nginx -t` при этом зелёный, `reload` проходит, и
   данные поиска с картами сайта просто **никогда** не получают своей политики — молча.
   Остальные пять — `\$uri` в комментарии и в `try_files`; там ошибка громкая (страницы
   начнут отдавать 404), но это свойство `try_files`, а не страховка.
