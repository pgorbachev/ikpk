...
**`bootstrap-vps.sh` теперь идемпотентен и запускается повторно.** Прежняя редакция
этого раздела запрещала повторный запуск: скрипт писал vhost целиком, только `listen 80`,
и снёс бы 443-блок с редиректом, добавленный certbot. Сейчас он сходится к объявленному
состоянию (`deploy/environments/<окружение>.env`) и обращается с посторонним по объявленной
политике: `merge` — привести объявленные директивы, не трогая остального; `refuse` —
остановиться (код 3). Обход отказа — `FORCE_VHOST=1`, и он снимает резервную копию сам.
Маркеры постороннего, которые обязаны пережить приведение, перечислены в
`VHOST_FOREIGN_MARKERS`; их исчезновение — отказ.
...
- Деплой сайта (`scripts/deploy-web.sh`) касается только `web` (Astro static). Система
  управления разворачивается провижинингом: артефакт собирается `scripts/build-cms-artifact.sh`,
  доставляется `scripts/bootstrap-vps.sh` через `CMS_ARTIFACT_SOURCE` и живёт релизами с
  атомарной сменой `current`, как и сайт.
