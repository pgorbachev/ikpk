...
пикселей цвета кнопки во всей её рамке (порог 25 %; вписанный круг даёт ~78 %, с раскрытой
панелью её тень сбивает до ~52 %, отсутствие кнопки даёт 0 %). Прогон, где хоть один кадр
не прошёл, завершается ненулевым кодом; итоговые доли по каждому кадру записаны последней
строкой каждого `shot-log-*.txt`. Постусловие появилось не для красоты — оно поймало
реальный дефект съёмки, см. «Что чинилось по ходу».
...
- **Демо-режим.** Чат на демо гасится (Q22), виджет отзывов отнесён к тому же классу, что
  Метрика, которая в демо отключается. Чем замещается погашенный виджет — дырой в ~840 px
...
- **Состояние метки до загрузки и без JavaScript.** Значение приходит с клиента; в мокапе
  оно зашито. У A и C это отдельная строка, которой просто нет; у B — пустое место посреди
  строки копирайта. Вопрос 1 ниже.
...
12. **Тёмная тема вскрыла дефект НЕ в мокапе, а в репозитории: роль `--text-primary` в
    ...
    В мокапе перевёл на `--color-dark-700`, который верен в обеих темах.
...
13. **Статическая сводка в A и C — протухающая копия чисел живого виджета.** «4,9»,
    ...
    Это тот же класс, из-за которого затевались Q23 и Q28. Вариант B этим не болеет — там
    числа только у виджета. Для A и C выбор придётся сделать явно: либо сводка без чисел,
...
доминирует; числа не дублируются.
...
- **Негативная проверка постусловия.** Мутация — `<style>.chatw{visibility:hidden}</style>`
  ...
  семинарского**, то есть проверка различает предмет. Контрольная сумма дерева `dist-a` до и
  после мутации совпала побайтово (`d8b74993…`); `git status` здесь слеп — `dist` в
  `.gitignore`.

## Что нашли независимые проходы

Ревью в двух исполнителях с разными уклонами, оба строго на чтение: Codex — соответствие
фактам, локальный проход на другой модели — полнота последствий.
