...
1. **Столкновение чата с секцией отзывов реально, и его цена — не «кнопка налезла», а
   ...
   на **71 %, 100 %, 79 %, 89 %** на 375. Разброс на десктопе объясняется не размером виджета,
...
2. **На телефоне сторона чата не спасает: панель Bitrix24 занимает почти весь экран.**
   ...
   решает задачу только на десктопе; на 375 виджет отзывов закрыт на 79 % и там, у D — на 89 %.
...
6. **Иконки соцсетей и строка «Опубликовано» не спорят ни в одном варианте.** Пересечение
   ноль везде; вертикальный зазор 302/302/149 px (A), 248/291/180 (B), 114/113/133 (C),
...
9. **Секция отзывов удлиняет главную заметно, и цена зависит от формы виджета, а не от
   места.** 6 026 px сегодня → 6 999 (A, +16 %), 6 426 (B, +7 %), 7 291 (C, +21 %),
   **6 570 (D, +9 %)** на 1280. Полный виджет с комментариями стоит ~840 px высоты; B платит
   420 px, потому что показывает только рейтинг; **D платит 509 px и при этом отзывы
...
20. **Форма ленты (D) даёт отзывы на сайте почти по цене их отсутствия.** Секция 509 px
    против 420 у B, где отзывов не видно вовсе, и против 938–1118 у A и C; страница +9 %
    против +7 % у B и +16…21 % у A и C. Раскрытая панель чата закрывает ленту на 32 % — вдвое
...
| **C** | перед финальной CTA-полосой | 70 % | 560×800, по центру | 1 118 px | **слева** внизу | фирменные цвета, полосой в подвале | отдельной строкой под соглашением |
| **D** | после «Преподавателей» | 71 % | **широкая лента, 4 карточки в ряд** | 509 px | справа внизу | монохром в кружках, 4-я колонка | отдельной строкой под соглашением |
...
(широкая лента) тоже переносится: её можно поставить на любое из трёх мест.
...
сводка: крупные 4,9, звёзды, «33 отзыва · 66 оценок», бейдж и кнопка в Яндекс.Карты. Справа
...
виджета полосой 28×60; раскрытая панель закрывает 48 % виджета на 1280 и 71 % на 375, а
...
### Вариант D — «лента карточек, как у kkpk.pro»

Форма подсмотрена у конкурента, который встраивает тот же официальный виджет Яндекса:
заголовок «Отзывы учеников», серый контейнер со скруглением, белая плашка с рейтингом
слева вверху, четыре белых карточки отзывов в ряд, каждая с именем, звёздами и обрезанным
текстом, и подпись «Виджет отзывов» в правом нижнем углу. Место секции, сторона чата и
подвал — как в A.

Чем платит: видно **четыре отзыва по три-четыре строки**, а не вертикальный список целиком —
за подробностями посетитель уходит на Яндекс.Карты; секция стоит на 71 % глубины, то есть
её видит только долиставший; на 375 карточки встают в столбец и секция вырастает до 973 px.

Что выигрывает: отзывы **видны на сайте** ценой всего 509 px против 938–1118 у A и C и
всего +9 % к высоте главной против +16…21 %; раскрытая панель чата закрывает ленту на 32 %
против 48 % у A; своей сводки нет, поэтому нечего дублировать и нечему протухать
(finding 13); форма знакома тем, кто видел её у конкурентов.
...
приходится разнородное. После добавления варианта D поле изменилось: он берёт почти всю
дешевизну B, но отзывы всё-таки показывает.
...
- **D** показывает отзывы за 509 px и +9 % высоты — почти цена B, у которого отзывов нет
  вовсе; панель чата закрывает ленту на 32 % против 48 % у A; своей сводки нет, поэтому нет
  и вопроса 4. Его цена — видно четыре отзыва по три-четыре строки, а не список целиком, и
  секция стоит на 71 % глубины.
- **A** — середина по всем осям и ни в одной не лучший.

**Рекомендую D.** Довод: единственная ось, где A и C выигрывают у него, — объём видимого
(вертикальный список против четырёх карточек), а платят они за это вдвое-втрое большей
секцией, худшей уживаемостью с чатом и статической сводкой, которая протухнет. B дешевле на
2 процентных пункта, но не показывает ни одного отзыва — а секция отзывов затевалась,
чтобы их показывать. Если объём видимого важнее цены — тогда **C** (он лучше A и по чату, и
по различимости иконок), и тогда же надо решить вопрос 4. Если D берётся, стоит рассмотреть
и перенос его ленты выше по странице: форма это позволяет, и глубина 71 % — её единственная
слабая сторона.
