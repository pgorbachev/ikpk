...
- **выбор сделан 23.08.2026** — [`docs/demo-2026-08-23-mockup-choice.md`](../../../demo-2026-08-23-mockup-choice.md):
  главная — лента D с фирменными иконками из C, страница семинара — C. Выбранная
  комбинация снята отдельно как **вариант E**, потому что среди четырёх исходных её не было
- исследуемый коммит: `origin/main@034e0508738e4697648e717c317adde9e3c09acf` (кратко
  `034e050`) — **база переехала 23.08.2026**, когда в `main` слили PR #153 «доделки после
  демо». Он меняет ровно те поверхности, которые мокап сравнивает: второй телефон в подвале
  и шапке (D12/D13), «Новости» → «Предложения» (D21), подписи кнопок главной (D10) и кнопку
  «Записаться на семинар» вместо «Зарегистрироваться» (D11). Поэтому пересняты **все пять**
  наборов, включая опорный: числа «как сегодня» на прежней базе описывали страницу, которой
  больше нет. Прежняя база — `e0c4d1d`
- ветка мокапов: `design/demo-followups-mockups`
- дата съёмки: 2026-08-23, все пять наборов сняты с одной базы `034e050`
...
**База переехала, и пересняты все пять наборов, включая опорный.** 23.08.2026 в `main`
слили PR #153: второй телефон в шапке и подвале, «Новости» → «Предложения», новые подписи
кнопок главной, «Записаться на семинар» вместо «Зарегистрироваться». Это ровно те
поверхности, которые мокап и сравнивает, поэтому оставить старые кадры «как сегодня» было
нельзя: они описывали бы страницу, которой больше нет. Что сдвинулось в числах: главная
6 026 → **6 063** px, подвал на 375 1 094 → **1 120**, начало описания на странице семинара
243 → **228** px на 1280 и 402 → **410** на 375. Раскладочные выводы при этом устояли —
разница между вариантами держится на форме виджета и месте секции, а не на этих десятках
пикселей.
...
(проверено `git diff --name-only 034e0508738e4697648e717c317adde9e3c09acf..HEAD`, то есть от
**текущей** базы ветки, а не от локального `main`). **SHA здесь обязан совпадать с базой
ветки, и прежняя редакция это условие нарушала:** после переезда на `034e050` тут осталось
сравнение с `e0c4d1d`, а такой diff даёт 72 пути вне каталога мокапов — весь PR #153, включая
`web/src`. Доказательство «ноль файлов» верно только относительно `034e050`; со старым SHA оно
утверждало о составе ветки то, чего в ней нет. Что сборку мокапы не ломают, следует из другого — все сборки зелёные, иначе
снимков бы не было.
...
6. **Иконки соцсетей и строка «Опубликовано» не спорят ни в одном варианте.** Пересечение
   ноль везде; вертикальный зазор 302/302/149 px (A), 247/291/180 (B), 114/114/133 (C),
   302/302/148 (D) на 1280/768/375. Вопрос, ради которого снималась вся главная, закрыт отрицательным ответом
...
7. **Замена текста иконками НЕ укорачивает подвал на десктопе.** Список из шести текстовых
   ...
   под друга: 1 120 → 1 013 (A), 1 037 (B), 1 022 (C), 1 013 (D).
...
9. **Секция отзывов удлиняет главную заметно, и цена зависит от формы виджета, а не от
   места.** 6 063 px сегодня → 7 036 (A, +16 %), 6 464 (B, +7 %), 7 391 (C, +22 %),
   **6 621 (D, +9 %)** на 1280. Полный виджет с комментариями стоит ~840 px высоты; B платит
...
14. **Сегодня расписание семинара лежит на двух третях глубины страницы, и кнопки записи на
    первом экране нет ни при каком числе дат.** Блок начинается на 2 763 px из 4 183 (66 %)
    на 1280 и на 5 286 из 8 367 (63 %) на 375; первая «Записаться на семинар» (так кнопка
    названа после D11, слитого в `main` 23.08.2026) — на 2 861 и 5 512. Во всех трёх
    вариантах она поднимается на первый экран: 326/569 (A), 249/527 (B), 397/513 (C)
    на 1280/375.

15. **Преимущество варианта C на планшете исчезает.** На 1280 боковая колонка не опускает
    описание вовсе (228 px, как сегодня). Уже на 768 колонка встаёт над текстом, и при семи
...
16. **Случай «без дат» — большинство (81 страница из 126, 64 %), и на нём варианты почти не
    ...
    назначены» плюс два телефона. При равном поведении описание начинается с 372 px (A),
    374 (B), 228 (C) на 1280 и с 606, 579, 608 на 375 — против 228 и 410 сегодня. **То есть
...
17. **Липкая колонка (C) сужает читаемую колонку с 1 168 до 792 px** — и это скорее выигрыш:
    ...
    самая короткая из всех на 1280: 3 959 px против 4 183 сегодня.
...
| **E** — выбрано | после «Преподавателей» | 71 % | то же, что D | 523 px | справа внизу | **фирменные цвета, 4-я колонка** | отдельной строкой под соглашением |
...
четвёртой колонке (как в A, B и D) законны и мокапа не требуют, это смена заливки глифа.
Отсюда практический ответ на частый вопрос: **взять иконки из C и всё остальное из другого
варианта можно.** Тремя решениями это и описывается — цвет глифа (монохром или фирменный),
оправа (кружок или без) и место в подвале (четвёртая колонка или центрированная полоса).
Сняты две их комбинации из шести: «монохром в кружках, четвёртая колонка» (A, B, D) и
«фирменные цвета без кружков, полосой» (C). Комбинация «фирменные цвета в кружках» не
снята — если нужна именно она, это отдельный кадр, а не выбор из имеющихся.
...
Что выигрывает: самая дешёвая по высоте — +401 px (+7 %) против +973 и +1 328; доверие
...
Чем платит: самая дорогая по высоте — +1 328 px (+22 %) на 1280 и +147 px подвала; отзывы
...
### Вариант D — «широкая лента: четыре отзыва в ряд» (требует стороннего сервиса)
...
### Вариант E — выбранная конфигурация: лента D плюс фирменные иконки C

Снят 23.08.2026 после выбора владельца. От D отличается **только цветом иконок соцсетей**:
фирменные вместо монохромных. Подвал остаётся четырёхколоночным — перестройка в три
колонки живёт в C и владельцем не выбиралась, это отдельная ось (см. «Оси можно смешивать
не все»).

Что это стоит: ничего по высоте на десктопе — главная те же 6 621 px, секция те же 523,
подвал те же 462. На 768 и 375 фирменные плитки на 3 px выше монохромных кружков, поэтому
секция 724 против 707 и 1 098 против 1 082, страница 11 536 и 12 869. Что это даёт:
Youtube и Rutube различаются сразу — в монохроме они почти неотличимы (finding 8).

**Оговорка та же, что у D:** такой раскладки официальный виджет Яндекса не даёт
(finding 25). Выбран облик; чем он выводится — сторонним сервисом или собственным
выводом — решается отдельно, вопросы 1 и 3 в [файле выбора](../../../demo-2026-08-23-mockup-choice.md).

![Вариант E, 1280, лента отзывов](variant-e/home-1280-02-reviews-block.png)
![Вариант E, 1280, подвал с фирменными иконками](variant-e/home-1280-04-footer-block.png)
![Вариант E, 1280, раскрытый чат поверх ленты](variant-e/home-1280-06-reviews-chat-open.png)
![Вариант E, 375, лента](variant-e/home-375-02-reviews-block.png)
![Вариант E, тёмная тема, лента](variant-e/home-1280d-02-reviews-block.png)

Главная целиком: [1280](variant-e/home-1280-01-full.png) · [375](variant-e/home-375-01-full.png).
Страница семинара у E не снималась: выбран вариант C, его кадры выше.
...
|---|---|---|---|---|---|---|
| **сегодня** | 228 px | 2 861 px, вне экрана | 285 px | 343 px | 4 183 px | 228 / 410 |
| **A** | 915 px (+687) | 326 px | 1 911 px (+1 626) | **1 969 px (+1 626)** | 4 227 px | 372 / 606 |
| **B** | 372 px (+144) | 249 px | **458 px (+173)** | **648 px (+305)** | 4 308 px | 374 / 579 |
| **C** | **228 px (0)** | 397 px | 1 447 px (+1 162) | **1 608 px (+1 265)** | **3 959 px** | **228 / 608** |
...
Чем платит: описание уезжает под расписание — на 915 px при семи датах на 1280, на 1 911 на
768 и на 1 969 на 375. Это худший сдвиг из трёх, и на телефоне он больше двух экранов.
На страницах без дат (81 из 126) A стоит вровень с B: 372 против 374 px.
...
на странице дважды — полосой и таблицей; страница становится самой длинной (4 308 px на
1280, 8 588 на 375); «ещё N дат» требует от посетителя второго действия.
Что выигрывает: **наименьший сдвиг описания из трёх** — и единственный, у которого он мал
на всех вьюпортах: +144 px на 1280, +173 на 768, +305 на 375. Последнее число честнее
...
десктопе** — на 768 описание уезжает на 1 447 px, на 375 на 1 608.
...
на 1280, +173 на 768, +305 на 375. Оговорка — как раз последнее число: на телефоне это 37 %
...
Платит C высотой: +1 328 px и глубина 70 %. **Это рекомендация со слабым отрывом**, и если
...
9. **Менять ли решение Q7/Q13 ради формы варианта D.** Четырёхкарточная лента официальным
   виджетом Яндекса не делается (finding 25). Дальше два пути, и **ни один из них не
   единственный** — прежняя редакция называла сторонний сервис единственным способом и тут же
   описывала третий ответ, то есть противоречила себе в одном абзаце. Путь первый: сторонний
   сервис-посредник — чужой скрипт на наших страницах, подписка, своя политика данных,
   отдельные решения по демо-режиму и куки. Путь второй: **выводить отзывы самим** —
   решениями он не закрыт (Q19 запрещает копировать чужие фото, про тексты в решениях ничего
   нет), но требует своих ответов о правах на тексты, условиях Яндекса, порядке обновления и
   отказе от аватаров. Ответ «нет» оставляет выбор между A, B и C и
...
- **У варианта D расписание семинара пропадало целиком.** Условие в шаблоне перечисляло
  варианты, которые показывают блок на своём месте (`off` и `b`), а не те, которые его
  переносят (`a` наверх, `c` вбок) — и D, добавленный позже, не попал ни в один список.
  Кадров это не задело (семинар у D не снимался), но патч воспроизводил дефект, а README
  утверждал, что у D страница семинара «как сегодня». Признак перевёрнут: блок показывается
  везде, кроме вариантов, которые его переносят. Тот же класс, что «список известных
  подкоманд» в `AGENTS.md`: перечисление отстаёт от предмета молча.
...
- **Сравнение смешивало раскладку с поведением.** Решение Q11 («уточняйте у менеджера» с
  ...
  без дат A и B стоят вровень** (на прежней базе 387 против 389 px, на нынешней 372 против
  374), и довод против A сжался до
...
- **«Не опуская описание» противоречило собственным числам:** на 375 описание сдвигается с
  343 до 648 px, то есть на 37 % высоты экрана. Формулировка заменена на «наименьший сдвиг
...
широкая лента из четырёх отзывов в ряд — форма, подсмотренная у конкурента.
...
### Круг шестой, по правкам пятого

Две находки, три исправленных места, отклонено ноль. Первая находка называла обе
формулировки про Q19 сразу — в патче и в README, — поэтому пунктов ниже три, а не два.

Общее у находок — **класс**, а не причина: обе про правку, доведённую не до всех артефактов,
но правки разные и рассинхрон в них разный. Первая: исправление про Q19 внесли в README и не
довели до патча и до живого кода мокапа. Вторая к пятому кругу отношения не имеет вовсе —
это след переезда на новую базу: ветка переехала на `034e050`, а команда проверки состава PR
осталась со старым `e0c4d1d`. Первая редакция этого вступления сваливала их в одну причину
(«правку пятого круга не довели»), и третий пункт ниже ей прямо противоречил; поправлено
седьмым кругом.

- **[P2] Ложный запрет Q19 остался в воспроизводимом патче.** Пятый круг поправил README, но
  `mockup-code.patch.txt` продолжал утверждать, что «решение Q19 прямо запрещает переносить
  их к себе». Проверено по `docs/demo-2026-08-19-decisions.md` на `034e050`: Q19 говорит
  только про фотографии. Это хуже обычной опечатки — патч восстанавливает **код**, то есть
  ложный довод переехал бы в реализацию вместе с ним. Исправлено и в патче, и в живом
  `web/src/components/mockup/ReviewsWidget.astro`, чтобы они не разошлись.

  Правка не может изменить снимки, и это проверено выводом, а не рассуждением: текст лежит в
  JSDoc внутри frontmatter-ограды, а поиск любой его части и слова `Q19` по всем пяти
  собранным деревьям (`dist-ref`, `dist-a`…`dist-d`) даёт ноль файлов. Пересъёмка поэтому не
  требуется — постусловия пятого круга остаются в силе.

- **[P2] Связанный хвост в README: «получить её можно только через сторонний сервис».** Тот
  же абзац двумя предложениями ниже описывал третий ответ — выводить отзывы самим. То есть
  документ называл путь единственным и сам себя опровергал. Переписано на «два пути, ни один
  не единственный».

- **[P2] Проверка состава PR ссылалась на прежнюю базу.** После переезда на `034e050` в
  доказательстве «вне каталога мокапов — ноль файлов» остался SHA `e0c4d1d`. Пересчитано:
  относительно `e0c4d1d` тот же diff даёт **72** пути вне каталога мокапов — весь PR #153,
  включая `web/src`; ноль получается только относительно `034e050`, и merge-base ветки именно
  он. Заменено на полный SHA с точной командой, а не на имя ветки: доказательство состава,
  привязанное к устаревшей базе, утверждает о ветке то, чего в ней нет.

### Круг седьмой, по правкам шестого

Одна находка, отклонено ноль.

- **[P3] Вступление шестого круга сваливало две находки в одну причину.** «Обе находки об
  одном: правку пятого круга внесли в README и не довели до остальных артефактов» — верно
  только для первой. Вторая (устаревший `e0c4d1d` в команде проверки состава) вызвана
  переездом на новую базу и с пятым кругом не связана; третий пункт того же раздела это и
  показывал, то есть абзац противоречил тексту под собой. Переписано: у находок общий
  **класс** — правка, доведённая не до всех артефактов, — а причины разные и названы
  порознь.
