...
25. **Форма варианта D — не режим официального виджета, а продукт стороннего сервиса.**
    ...
    **iframe Яндекса на странице нет вовсе**, от Яндекса там только обычная ссылка
    ...
    ровно то, против чего finding 23; **(2)** рисовать ленту самим, но тогда тексты чужих
    отзывов пришлось бы держать у себя, а это запрещает Q19. **Поэтому D убран из
    рекомендации** и оставлен как облик для оценки — если решение меняется, он возвращается
...
9. **Менять ли решение Q7/Q13 ради формы варианта D.** Четырёхкарточная лента официальным
   ...
   отдельные решения по демо-режиму и куки. Ответ «нет» оставляет выбор между A, B и C и
   ничего больше не требует; ответ «да» — это новое решение и отдельный change, а не правка
   оформления.
...
- **[P1] Вариант D основан не на официальном виджете.** Раздел D утверждал, что конкурент
  ...
  Q7/Q13 — либо запрещённого Q19 переноса чужих текстов к себе. **Рекомендация переписана:
