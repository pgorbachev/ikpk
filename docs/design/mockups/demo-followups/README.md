...
блока. Проверяются и вычисленные `visibility`, `display`, `opacity`: узел, схлопнувшийся в
ноль, уехавший за границу секции или спрятанный, разметку прошёл бы, а на снимке его бы не
было. Подписи ищутся по тексту, а не по числу элементов: список наград
...
20. **Форма ленты (D) даёт отзывы на сайте почти по цене их отсутствия.** Секция 523 px
...
24. **Виджет Яндекса приносит с собой счётчик Метрики.** В ответе виджета лежит пиксель
    `mc.yandex.ru/watch/57020224` — счётчик Яндекса, не наш. То есть встраивание — это не
    только `<iframe>` с отзывами, но и третья сторона со своей аналитикой внутри страницы.
    Гашение на демо уже решено; здесь важно другое — это факт для change «внешние виджеты»
    и для текста о куки, а не деталь оформления.
---
...
| **D** | после «Преподавателей» | 71 % | **широкая лента, 4 карточки в ряд** | 523 px | справа внизу | монохром в кружках, 4-я колонка | отдельной строкой под соглашением |
...
её видит только долиставший; на 375 карточки встают в столбец и секция вырастает до 1 082 px.
...
- **D** показывает отзывы за 523 px и +9 % высоты — почти цена B, у которого отзывов нет
...
9. **Ставить ли знак 2ГИС вообще и кто обновляет год.** Официальных марок для сайта нет ни у
