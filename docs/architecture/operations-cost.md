...
─────────────────────────────────────────────────────────────────
Запросов на VPS:          все (~1000/сут)     только CMS-админка (~50/сут)
...
Rebuild при обновлении:   —                   ~3 сек сборки, ~1 раз/день
```

**Вывод:** нагрузка минимальная. Бесплатных тарифов CDN хватает с большим запасом.
VPS для Strapi будет загружен на ~5% — основная нагрузка это rebuild при сохранении контента.
