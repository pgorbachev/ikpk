...
| # | Task | Status | FR/NFR | Depends on |
|---|------|--------|--------|------------|
| 4.1 | Pagefind search (build-time index, lazy load) | ✅ | FR-05 | индекс в `npm run build`; UI лениво при первом открытии; псевдо-поиск (GET /statyi) заменён; e2e с опечаткой |
