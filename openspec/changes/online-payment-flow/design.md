...
ключа / несовпадение содержимого / неизвестный `requestId` с совпавшим отпечатком; Решение
...
шаги 1а, 5 и 6 «Итогового порядка» Решения 2а. На шаге 1а поле ставится на **совпавшую по
отпечатку (каноническую) запись**, потому что записи по присланному `requestId` не
существует и создавать её нечем; это же и есть та запись, чей `requestId` уходит клиенту
(см. ниже, «Канонический `requestId` в ответе»).

**Приоритет при чтении.** `GET` отвечает `verification_required`, если `verificationAt`
стоит **и** статус записи не терминален. Терминальный статус старше: как только запись
...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   ...
   **каноническим `requestId` этой совпавшей записи** (Решение 2е, «Канонический
   ...
   поле и кто его ставит») и пишет журнальную запись. Поиск по отпечатку и создание новой
...
**Во всех трёх ветвях, отвечающих `verification_required` (шаги 1а, 5 и 6), сервер той же
операцией ставит на запись состояние «нужна сверка»** — `verificationAt` и
`verificationReason` (Решение 2, «Хранимое поле и кто его ставит») — и пишет журнальную
запись (Решение 2е). Разница только в том, на какую запись: на шаге 1а — на совпавшую по
отпечатку каноническую, на шагах 5 и 6 — на найденную по присланному `requestId`. Без этой
записи `GET .../status` после перезагрузки не смог бы воспроизвести исход, и вся защита
держалась бы на памяти браузера.
...
1. **Клиентское удержание.** Браузер постоянно хранит `requestId` последней попытки и
   время её создания; при загрузке `/oplata` страница спрашивает у сервера состояние этой
...
**Верхняя граница второго слоя — ровно 14 суток с момента создания совпавшей записи, и это
отдельное правило, а не следствие TTL.** Хранение записи и журнала задано как **минимум**
(«не менее 14 суток»), то есть реализация вправе держать их дольше; блокировка же по
отпечатку дольше держаться не вправе — иначе состав данных оказался бы занят на срок, не
ограниченный ничем, и посетитель, которого никто не разблокировал, не смог бы заплатить
никогда. Поэтому граница блокировки записана точным числом, а не ссылкой на жизнь записи:
на 14 сутках и позже тот же состав под новым `requestId` проходит как новая попытка, даже
если сама запись ещё жива.
...
Ручное снятие удержания оператором (Решение 2ж) отдельного, четвёртого основания не
добавляет: команда переводит запись в терминальное состояние, и удержание снимается по
основанию (1) — тем же путём и той же проверкой, что после обычного отказа платежа. Это
намеренно: новый способ повлиять на клиента означал бы новую клиентскую ветвь, а её никто
не проверял бы до боевого запуска. **Решение 2ж отложено (Решение 2з), и на перечень
оснований это не влияет вовсе** — именно потому, что четвёртого основания оно не вводило:
клиентская часть здесь остаётся без изменений, отложена только серверная команда.
...
**Разблокировка после ручной сверки — механизм выбран (Решение 2ж) и тем же днём отложен
до ответа заказчика (Решение 2з); уполномоченный и критерий заключения остаются открытыми.** Записи, которые автоматический выход не разрешает
(нет идентификатора платежа и прошло окно дедупликации; либо отпечаток не совпадает, а
платёж не завершён), требуют человека. **Каким действием** удержание снимается — решено
владельцем 2026-08-13: операторской командой на платёжном сервисе, с обязательной причиной
и записью в журнал (Решение 2ж). **Кто именно** уполномочен и **чем подтверждается**
заключение «платежа по попытке нет» — по-прежнему открыто и намеренно не додумано за
владельца: первое — внешняя роль на стороне заказчика, кодом не назначаемая (у change нет
интерфейса сотрудника, это названо в Решении 2д как причина не вводить двухстадийный
платёж), второе — операционный критерий, который из репозитория не выводится. Оба остались в
Open Questions. **Дополнено Решением 2з тем же днём:** сама команда отложена до ответа
заказчика и до тех пор **не существует** — прежняя формулировка «команда существует и
работает, открыт лишь вопрос эксплуатации» больше не верна, и заменена намеренно, а не
уточнена. Единственный выход из этого состояния до ответа — TTL записи (14 суток,
Решение 4), после которого тот же состав данных снова принимается.
...
### Решение 2ж: ручное снятие удержания — операторская команда сервиса, причина обязательна

> **ОТЛОЖЕНО более новым решением владельца от 2026-08-13 (см. Решение 2з).** Это решение
> остаётся в change как **выбранное направление**, но не как поставка: реализация по нему
> **не начинается**, и в production-контракт оно не входит до ответа заказчика о текущей
> связи сайта, Bitrix и ЮKassa. Причина — тот же день, более узкий пункт: «пока не
> проектировать операторский workflow». Ручное снятие удержания оператором есть именно
> операторский workflow, поэтому под пункт попадает целиком.
>
> История решения не переписывается: ниже читается ровно та формулировка, которой решение
> было принято утром 2026-08-13, и она остаётся нормативной как описание направления. Что
> изменилось — только статус: было «решено, к реализации», стало «решено как направление,
> реализация отложена». Второй абзац Решения 2з называет, какая часть от этого снова стала
> вопросом без объективного критерия.
...
свойством одного ответа). Персональных данных ни одно из них не содержит:
`verificationReason` — значение из перечня трёх причин, `verificationAt` — отметка времени.
Имя,
...
- **Серверный контроль по отпечатку «занимает» состав данных — но не дольше 14 суток
  ...
  автоматического выхода нет: остаётся ручная сверка — операторская команда с обязательной
  причиной (Решение 2ж) — либо верхняя граница блокировки, ровно 14 суток с создания
  совпавшей записи. Открытым в регламенте сверки осталось не «каким действием», а кто
  уполномочен и чем подтверждает заключение. **Уточнено Решением 2з (13.08, более новое):**
  команда отложена и до ответа заказчика не строится, поэтому на сегодня выход из этого
  состояния ровно один — те же 14 суток. Риск от этого не исчезает и не уменьшается: он
  ровно такой, каким был до Решения 2ж, и здесь назван именно поэтому.
...
- **Как устроена сегодняшняя связь сайта, Bitrix и ЮKassa?** Открыто 2026-08-13, ответ
  запрошен у заказчика со скриншотами, ожидается в течение 1–2 дней. Пять неизвестных
  перечислены в Решении 2з и здесь не дублируются. Это вопрос **вне репозитория и вне
  production-наблюдения из браузера**: видна только клиентская половина цепочки. От ответа
  зависит production-контракт эндпоинта и архивирование change (`tasks.md`, 2.8 и 7.8), а не
  оформление уже написанного.
- **Кто уполномочен снимать удержание после ручной сверки и чем подтверждается его
  заключение?** Записи, которых автоматический выход не разрешает (нет идентификатора платежа
  ЮKassa и прошло окно дедупликации; либо отпечаток не совпадает при незавершённом платеже),
  требуют человека. **Каким действием** удержание снимается — направление выбрано владельцем
  2026-08-13 утром: операторская команда сервиса, обязательная причина, запись в журнал
  (Решение 2ж). **Тем же днём этот подвопрос вернулся в открытые более новым решением
  (Решение 2з):** операторский workflow пока не проектируется, а объективного критерия
  «какой ручной выход институту нужен» у нас нет — он зависит от пятого неизвестного
  (существует ли уже операционный порядок разбора таких случаев на стороне института). То
  есть закрытым этот подвопрос считать нельзя: выбранное направление записано, но проверить
  его правильность нечем до ответа. Остаются и два прежних, оба вне репозитория:
  ...
  **(2)** чем это заключение подтверждается — записью в панели, её отсутствием, выпиской:
  ...
  какой редакции. Пока ответа нет — и пока команда отложена Решением 2з, то есть **не
  существует** — единственным выходом остаётся TTL записи
