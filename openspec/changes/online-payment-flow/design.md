...
1. **Клиентское удержание.** Браузер хранит `requestId` последней попытки, время её
   создания, последний сообщённый исход и значения полей; при загрузке `/oplata` страница
   проверяет статус этой попытки и либо восстанавливает удержание с сохранённым исходом,
   либо снимает его. Что именно хранится, с какого момента и по каким основаниям снимается —
   ниже, «Что хранит удержание» и «Снятие удержания»; срок — 14 суток с момента создания
