...
версии), нарушает допущение сразу в двух местах: реплики могут получить разные ключи
HMAC при не идеально одновременной ротации (сервер А уже видит новый ключ как
`CURRENT`, сервер Б — ещё старый как `CURRENT`, и версии перестают совпадать по
смыслу), и обе реплики пишут в разные копии локального хранилища, если оно не общее.
...
- **Переменная не задана — сервис SHALL NOT запускаться.** **Найдено ревью (P1-2 после
  merge #90, владелец — не путать с P1-2 из ревью до merge про отпечаток запроса,
  Решение 3а): первая редакция трактовала `false` и отсутствие переменной одинаково —
  «нет receipt».** Это fail-open по вопросу, регулируемому 54-ФЗ: забытая переменная
  окружения на проде молча отключила бы чеки, и никто не увидел бы явного сигнала о
  том, что решение не принято, а не принято намеренно. Отсутствующее значение — это
  «неизвестно», не «выключено», и раз неизвестность здесь стоит дорого (юридическое
  требование, не техническая деталь), сервис отказывается стартовать вместо того, чтобы
  молча выбрать сторону. Сообщение при отказе называет причину и оба допустимых
  значения — по прецеденту, уже принятому в проекте для `DEPLOY_MODE`
  (`scripts/deploy-web.sh`: «Не задан `DEPLOY_MODE`… Умолчания нет намеренно: любая из
  двух ошибок молча теряет заявки либо пишет тестовые обращения в CRM заказчика» — тот
  же класс решения, применённый здесь к чекам вместо адреса форм).
- **Область действия проверки — обработчик, который обращается к ЮKassa, не сервис
  целиком.** Найдено ревью (minor #97, владелец): fail-closed выше нужен там, где
  решается вопрос 54-ФЗ, — это боевой обработчик `POST .../payments`. Демонстрационный
  обработчик (Решение M10, `specs/online-payment/spec.md`) к ЮKassa не обращается ни
  при каком значении `RECEIPT_ENABLED` и к чекам отношения не имеет вовсе — требовать
  от него ту же переменную означало бы, что демо-стенд отказывает в запуске по причине,
  которая его не касается. `RECEIPT_ENABLED` проверяется при первом обращении к
  боевому обработчику (или при старте, если конфигурация сборки уже говорит, что это
  боевой контур), а не как условие запуска процесса как такового.
...
**раньше** статуса — а Решение 3б трактует запись с устаревшей (пережившей две
...
**Исправление: проверка терминального статуса предшествует проверке отпечатка, а не
наоборот.**
...
4. Если запись найдена и её статус **не `succeeded`** (терминально отменён,
   не завершён, статус неизвестен) — только тогда проверяется отпечаток
   (Решение 3а) и, если он совпадает по `keyVersion`, применяется обычная логика
   повтора; несовпадение содержимого или `keyVersion` старше `PREVIOUS` дают `409`,
   как и раньше.

Отпечаток защищает от подмены содержимого у **незавершённых** попыток — у него нет
причины и полномочия отменять уже случившееся `succeeded`. Формально это тот же
`already_paid` без каких-либо предварительных условий, что и Requirement выше уже
называет SHALL: исправление приводит реализацию (Решение 3а/3б) в соответствие с уже
принятым безусловным контрактом, а не ослабляет его.
...
`succeeded` (см. Решение 2а, «BLOCKER, найдено ревью #97»).** Уже подтверждённый
платёж возвращает `already_paid` безусловно, раньше, чем отпечаток и `keyVersion`
вообще вычисляются для сравнения — отпечаток защищает незавершённые попытки, у него
нет полномочия отменить уже случившееся `succeeded`.
...
### Решение 3б: ротация ключа HMAC — версионированный ключ, не мгновенная замена

**Найдено ревью (P1-1 после merge #90, владелец — не путать с P1-1 из ревью до merge
про срок регистрации тестового магазина): простая замена ключа (как в Решении 7 для секрета
ЮKassa) ломает идемпотентность.** Отпечаток хранится до 14 суток (Решение 4). Если
ключ HMAC заменить простой заменой файла, тот же самый повторный запрос после ротации
даст другое значение `fingerprint`, сервер сочтёт его несовпадающим с сохранённым и
отклонит `409` — легитимный повтор (например, после «исход неизвестен») окажется
заблокирован собственной защитой от подмены содержимого.

**Решение: версия ключа хранится вместе с отпечатком.**

- Сервер держит **не более двух** ключей HMAC одновременно: `HMAC_KEY_CURRENT` (для
  вычисления отпечатка новых запросов) и, во время переходного периода после ротации,
  `HMAC_KEY_PREVIOUS` (только для проверки уже существующих записей — новые отпечатки
  им не считаются).
- Запись состояния (Решение 4) хранит `fingerprint` вместе с `keyVersion` —
  идентификатором того, каким из двух ключей он посчитан.
- При проверке повторного запроса сервер выбирает ключ по `keyVersion` записи (текущий
  либо предыдущий), а не всегда текущий — иначе проверка после ротации отклоняла бы
  все старые записи независимо от того, изменилось ли содержимое.
- **Минимальный интервал между ротациями — не короче TTL записи (14 суток).** Пока
  предыдущий ключ ещё нужен действующим записям, хранилище занято парой
  «текущий/предыдущий»; следующая ротация до истечения этого окна вытеснила бы
  предыдущий ключ раньше, чем истекут ссылающиеся на него записи, и вернула бы ту же
  проблему на одно поколение позже. Runbook ротации (задача 4.5) называет это правило
  явно, а не оставляет угадывать.

**Внеплановая (аварийная) ротация раньше этого окна** — например, при подозрении на
компрометацию ключа — принимается как осознанный компромисс, а не запрещается: записи
с `keyVersion`, не совпадающей ни с текущим, ни с предыдущим ключом (то есть пережившие
две ротации подряд), у которых статус **не `succeeded`**, SHALL трактоваться как
несовпадение отпечатка — тот же `409`, что и при подмене содержимого. Для таких
записей отдельная сознательная отправка с новым `requestId` (Решение 2а) остаётся
доступна; теряется только «бесплатная» идемпотентность конкретного зависшего повтора,
а не платёж как таковой.

**Уточнение по BLOCKER ревью #97:** правило выше касается только незавершённых
записей. Запись со статусом `succeeded`, независимо от возраста её `keyVersion`,
возвращает `already_paid` — устаревшая версия ключа никогда не проверяется для уже
подтверждённого платежа (см. Решение 2а). Аварийная ротация может «состарить» отпечаток
незавершённой попытки, но не может превратить состоявшуюся оплату в отказ.
...
ротацию без потери идемпотентности, и аварийную — с точно названной, а не скрытой
ценой.
...
**Ротация секрета ЮKassa (найдено ревью, m5).** Хост, на котором живёт сервис, уже несёт
