...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   ...
   **живые незавершённые** записи (живая блокировка по отпечатку: созданы не ранее 14 суток
   ...
   считается. Только если живого совпадения среди незавершённых нет — **подтверждённые не
...
если живой подтверждённой записи того же состава нет** — даже если сама незавершённая запись
ещё существует.
