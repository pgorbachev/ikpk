...
(`docs/decisions/payment-runtime-options.md`, слит #107).** Принятая форма — та же, что
...
отпечатка — файл окружения, принадлежащий отдельному системному пользователю сервиса**
...
| `503` | `{ "status": "verification_required", "requestIdKnown": true \| false }` | статус записи не терминален (`pending`/`unknown`), и либо версия ключа не входит в число хранимых, либо содержимое не совпало (Решение 2а, шаги 5–6; Решение 3б, «Третий исход») — не `409`, `requestId` не заменяется автоматически. **Тот же ответ на новый, неизвестный серверу `requestId`, отпечаток которого совпал с незавершённой записью** (Решение 2е, шаг 1а «Итогового порядка»), но с `requestIdKnown: false`. Поле обязательно и различает два случая, у которых разные последствия у клиента (Решение 2е, «Два вида „нужна сверка“»): `true` — запись по присланному `requestId` есть, платёж по нему мог быть создан; `false` — записи нет и не появится, платёж по присланному `requestId` создан не был |
...
Ответ: `{ "status": "pending" | "succeeded" | "canceled" | "unknown" }` при `200`, и
**`404` с телом `{ "status": "not_found" }`, если записи по этому `requestId` у сервера
нет**. Идентификатор запроса в пути — `requestId`, а не `id` платежа ЮKassa: страница
...
**`verification_required` в домен ответа `GET` НЕ добавляется, и это выбор, а не
упущение.** Исход «нужна сверка» рождается сравнением *присланного содержимого* с записью
(Решение 2а, шаги 5–6) — у `GET` содержимого нет, воспроизвести это сравнение он не может,
а хранить на записи флаг «требует сверки» значило бы добавить поле и держать его
согласованным с двумя разными причинами исхода. Вместо этого исход, однажды сообщённый
клиенту, хранит **клиент** вместе с удержанием (Решение 2е, «Что хранит удержание»), а
`GET` отвечает на единственный вопрос, который к нему относится: завершилась попытка или
нет. Следствие для проверок: после перезагрузки состояние восстанавливается из удержания,
а не из ответа `GET`, — и требование это описывает именно так.
...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   совпадение по отпечатку среди незавершённых записей** (внесено Решением 2е): если
   отпечаток содержимого запроса совпадает с отпечатком незавершённой записи, версия
   ключа которой известна серверу, — ответ `verification_required` с
   **`requestIdKnown: false`** (записи по присланному идентификатору нет — Решение 2е, «Два
   вида „нужна сверка“»), платёж не создаётся. Поиск по отпечатку и создание новой записи
   выполняются атомарно (там же, «Поиск по отпечатку обязан быть атомарным»).
...
2. Если совпадения нет ни по `requestId`, ни по отпечатку — обычное создание (проверка
   формата, границ суммы, вызов ЮKassa).
...
5. Если запись найдена, её статус **не терминален** (`pending` или `unknown`), и
   ...
   `requestIdKnown: true`, а запись становится предметом ручной сверки** (уточнено вторым
...
| нетерминальный | известна | совпало | нет | за пределами окна (> 24 ч) | 5 | `verification_required`, `requestIdKnown: true` — ручная сверка | **нет** |
...
1. **Клиентское удержание.** Браузер хранит `requestId` последней попытки и признак её
   незавершённости; при загрузке `/oplata` страница проверяет статус этой попытки и либо
   восстанавливает удержание, либо снимает его по завершённому исходу. Срок удержания не
   переживает серверную запись (14 суток, Решение 4): удержание, пережившее запись,
   ссылалось бы на попытку, о которой сервер уже ничего не знает, и посетитель получил бы
   вечный тупик вместо защиты.
2. **Серверный контроль по отпечатку.** Сервер отклоняет создание платежа по **новому**
   `requestId`, если отпечаток содержимого совпадает с отпечатком существующей
...
записи (Решение 4), нужен лишь поиск по отпечатку среди незавершённых записей.
...
«Автоматический выход» ниже.

**Второй раунд ревью #112 (Б2) нашёл здесь вторую сторону, о которой прошлая редакция
утверждала обратное.** Клиент, чей `requestId` сервер отклонил по отпечатку, записи не
имеет и никогда не получит: `GET .../status` по его идентификатору отвечает `not_found`
всегда. Значит для него «проверка статуса разрешает попытку» не работает по построению —
удержание, поставленное по такому ответу, не снялось бы ничем до истечения срока. Оплатил
с ноутбука, зашёл с телефона — и до 14 суток без формы. Дефект был не в самом слое, а в
том, что клиент трактовал оба вида «нужна сверка» одинаково; разведены они ниже.

**Автоматический выход для записи без идентификатора платежа.** До этого update такой
...
идемпотентности, а не отвечает из хранилища. Это безопасно по построению и не новая
...
находка «момент создания удержания не назван»).** Удержание хранит четыре вещи:
`requestId` попытки, **время её создания**, **последний сообщённый сервером исход** и
значения полей (ради сводки — ниже). Момент создания удержания — **отправка запроса на
...
Хранение **исхода**, а не только признака незавершённости, — это то, чем закрывается Б1.
`GET .../status` отвечает `pending`/`unknown`, и по одному этому ответу страница показала бы
«статус пока неизвестен» с активной кнопкой: состояние без предупреждения «не оплачивайте
повторно», то есть ровно то, ради чего третий исход вводился. Поэтому после перезагрузки
исход берётся из удержания, а `GET` отвечает лишь на вопрос «завершилась ли попытка».
...
несёт элемента отправки. Прежняя формулировка через отрицание («не форма») была зелёной на
деградировавшем поведении: «статус неизвестен» — не форма.

**Снятие удержания — три основания, все объективные.** (1) `GET .../status` вернул
...
слой по отпечатку опирается на то же хранилище и слепнет вместе с ним); (3) истекли
...
**Два вида «нужна сверка», и клиент обязан их различать (внесено по Б2).** Ответ несёт
`requestIdKnown` (Решение 2, таблица кодов):

- `requestIdKnown: true` — запись по присланному `requestId` есть, платёж по нему мог быть
  создан. Клиент **удерживает** попытку и показывает идентификатор как код, по которому
  сотрудник найдёт платёж (`metadata.requestId`/`description`, см. «Что уходит в ЮKassa»);
- `requestIdKnown: false` — сервер отказал новому идентификатору по совпадению отпечатка.
  Платёж по нему **не создавался**, записи нет и не будет, `GET .../status` навсегда
  `not_found`. Клиент **не удерживает** такую попытку: удерживать нечего, а удержание стало
  бы тупиком. Предупреждение «платёж по этим данным мог быть создан, не оплачивайте
  повторно» и контакты остаются — опасность-то настоящая, просто чужая этому
  идентификатору. Повторная отправка снова получит тот же отказ, пока живёт совпавшая
  запись: это честный повторяемый отказ, а не запертая форма, и второго платежа он не
  создаёт (его и отклоняет сервер).

**Идентификатор при `requestIdKnown: false` не показывается как код поиска платежа —
третья находка ревью #112 (P1).** В ЮKassa лежит `requestId` **другой** записи, и
продиктованный сотруднику код не найдёт ничего: оператор заключил бы, что платежа нет.
Сообщать посетителю чужой идентификатор нельзя (он не его), поэтому в этом случае опознание
идёт по сводке данных попытки — ФИО, семинар, сумма, время, — которые в панели видны в
`description`. Именно на этом стыке открывается отложенная развилка «короткий код»: если в
панели нельзя искать по `description`, у этой ветви не остаётся ничего.
...
**Цена удержания в персональных данных названа, а не оставлена расхождением (внесено по
P1: контракт расходился между `design`/`tasks` и нормативной частью).** Удержание хранит
ФИО, телефон, почту, семинар, сумму и время в браузере **посетителя** — до снятия
удержания, то есть до 14 суток. Это не хранение у нас: данные лежат на устройстве того, кто
их ввёл, сервер их не получает и не журналирует (Решение 4). Тем не менее это хранение ПДн
на срок, и оно нормативно: спека требует и удерживать значения, и **стирать их в момент
снятия удержания**. Условная формулировка «если страница удерживает» из первой редакции
позволяла реализации не удерживать вовсе — и тогда сводка, ради которой всё делалось, не
появлялась бы никогда, а требование о ней оставалось бы формально выполненным. Текст
согласия обязан покрывать этот срок (`tasks.md`, 2.6).
...
объективно, — **журнал**: каждый ответ `verification_required` оставляет на сервере запись
с `requestId`, временем, причиной (неизвестная версия ключа / несовпадение содержимого /
неизвестный `requestId` с совпавшим отпечатком) и **без персональных данных** (Решение 7 —
секреты и ПДн не журналируются). Кто и как часто читает этот журнал — часть уже открытого
вопроса о ручной сверке, а не отдельная развилка.

**Фокус и объявление исхода (внесено по находке ревью #112).** Исход **вытесняет** форму
внутри уже открытого окна, в том числе кнопку отправки, на которой в этот момент стоял
фокус. `request-form-transport` описывает доступность **окна** (открытие, ловушка фокуса,
Escape) и о смене содержимого внутри окна не говорит — то есть после замены фокус
оказывался на удалённом элементе, а пользователь программы чтения с экрана не узнавал об
исходе вовсе. Отсюда требование: фокус переносится внутрь панели исхода, и текст исхода
объявляется вспомогательной технологии. Проверяется двумя измеримыми признаками —
`document.activeElement` внутри панели исхода после перехода, и объявляющая роль/область на
самой панели, — а не «выглядит доступно».

**Разблокировка после ручной сверки — открытый вопрос, а не умолчание.** Записи, которые
автоматический выход не разрешает (нет идентификатора платежа и прошло окно
дедупликации; либо отпечаток не совпадает, а платёж не завершён), требуют человека. Кто
этот человек, чем он подтверждает исход и каким действием снимает удержание — объективно
не определяется из репозитория: у change нет интерфейса сотрудника (это названо в Решении
2д как причина не вводить двухстадийный платёж), а доступ к панели ЮKassa — внешняя роль,
не решённая ни одним из четырёх решений владельца. Вопрос вынесен в Open Questions
целиком, а не переформулирован под то, что удобно проверить. До ответа действует
единственное объективное правило, которое у нас есть: запись истекает по TTL (14 суток,
Решение 4), после чего тот же состав данных снова принимается. Это не ответ на вопрос —
это то, что происходит, если на него не отвечать.
...
единственный их источник — браузер посетителя. Отсюда: удержание хранит и значения полей,
показывает их как **читаемую сводку**, и стирает их вместе со снятием удержания. Сводка
объявлена дополнением, а не условием: после очистки хранилища или в другом браузере её не
...
остальными наблюдениями (`tasks.md`, 2.2a); ответа заказчика на сегодня нет.
...
- **Клиентское удержание незавершённой попытки не переживает смену браузера, очистку
  ...
  поля даёт другой отпечаток и проходит. Остаточный риск назван, не устранён;
  обнаруживается постфактум там же, где M12, — по `description`/`metadata` в панели
  ЮKassa.
- **Серверный контроль по отпечатку «занимает» состав данных на время жизни записи
  (P3, Решение 2е)** → для записи с идентификатором платежа тупика нет (проверка статуса
  разрешает её сама); для записи без идентификатора введён автоматический выход —
  повтор к ЮKassa с тем же ключом идемпотентности. За пределами окна дедупликации ЮKassa
  выхода нет автоматического: остаётся ручная сверка, порядок которой — открытый вопрос,
  и TTL записи как единственное объективное правило до ответа.
...
1. Владелец решил все четыре развилки `proposal.md` (прод-origin — `api.ikpk.su`;
   ...
   1 000–200 000 ₽) и TTL (Решение 4, 14 суток). Рантайм подтверждён отдельно
...
**Открыто с 2026-08-12 (P3) — в отличие от трёх пунктов выше, это не «число, которое
можно скорректировать позже», а вопрос без ответа, от которого зависит поведение:**

- **Кто и на каком основании снимает удержание после ручной сверки?** Записи, которых
  автоматический выход не разрешает (нет идентификатора платежа ЮKassa и прошло окно
  дедупликации; либо отпечаток не совпадает при незавершённом платеже), требуют человека.
  Объективный критерий из репозитория не выводится: интерфейса сотрудника у change нет —
  его отсутствие названо в Решении 2д как причина не вводить двухстадийный платёж, — а
  доступ к панели ЮKassa остаётся внешней ролью (`proposal.md`, Ограничение объёма №2, где
  открыты именно роли). Три вопроса, на которые нужен ответ владельца и заказчика:
  **(1)** кто смотрит панель ЮKassa и вправе заключить, что платежа по попытке нет;
  **(2)** чем это заключение подтверждается — записью в панели, её отсутствием, выпиской;
  **(3)** каким действием удержание снимается — операторской командой на сервисе,
  указанием посетителю, или никак (ждём TTL); **(4) добавлено вторым раундом ревью #112:**
  кто и с какой периодичностью читает журнал исходов «нужна сверка» (сам журнал — уже
  требование, см. Решение 2е; читателя журнала кодом не назначить). Пока ответа нет, действует TTL записи
  (14 суток, Решение 4): это не решение вопроса, а то, что происходит, если на него не
  отвечать, — и оно записано именно так, чтобы не быть принятым за ответ.
