...
идентификатору, а предупреждение обязательно для **всех** удержанных состояний, **кроме
состояний, доказывающих отсутствие платежа** (отправка, отклонение по проверке, лимит частоты,
демо, вопрос о повторной оплате). Заодно перестаёт засоряться журнал: в него попадали обычные двойные отправки, и
...
страницы клиент запрашивает статус каждого активного идентификатора; лимит частоты на `GET`
(Решение 5б) разрешает не менее 8 запросов на цикл загрузки, чтобы предельные 5 удержаний в
него укладывались.
...
несёт элемента, создающего новую попытку **вместо** удерживаемой — **кроме состояний, которые
доказывают отсутствие платежа:** отправка; отклонение по проверке; лимит частоты; демо;
вопрос о повторной оплате. В этих состояниях предупреждение ложно, а элемент продолжения
...
- версия встречается впервые (в хранилище canary нет записи для неё) **и хранилище отпечатков пусто** — сервер
  сохраняет вычисленный `canary` и продолжает запуск: это первое использование
  версии, эталон ещё не установлен;
- версия встречается впервые, но хранилище отпечатков **непусто** — то же, что отсутствие
  свидетельства при существующих отпечатках: fail-closed, процесс не запускается;
...
дороже по последствиям), но SHALL существовать у обоих. На `GET` лимит SHALL разрешать
**не менее 8 запросов на цикл одной загрузки** страницы: при максимуме 5 удержаний (Решение 2л)
одна загрузка обязана проверить статус каждой попытки, и отказ по лимиту сохранял бы
