...
| `duplicateConfirmed` | boolean | нет | только `true`, если задан; означает явное подтверждение повторной оплаты того же состава (Решение 2к). **В отпечаток не входит** — иначе подтверждение меняло бы отпечаток и обходило саму проверку |
...
шаги 5 и 6 «Итогового порядка» Решения 2а, и шаг 1а с оговоркой ниже.
...
идентификатору, а предупреждение обязательно для **всех** удержанных состояний, включая «оплата
ещё идёт». Заодно перестаёт засоряться журнал: в него попадали обычные двойные отправки, и
...
| `409 duplicate_confirmation_required` | «по этим данным оплата уже подтверждена — создать ещё один платёж?» с явным подтверждением; при подтверждении повтор идёт с тем же новым `requestId` и `duplicateConfirmed: true` (Решение 2к) |
...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   ...
   решением владельца от 2026-08-13). Порядок внутри шага: сначала **подтверждённые** записи
   младше 14 суток — при совпадении и **отсутствии** `duplicateConfirmed` ответ
   `409 duplicate_confirmation_required`, платёж не создаётся, запись не меняется (Решение 2к);
   при наличии `duplicateConfirmed` — обычное создание. Затем **незавершённые** записи: если
...
**В ветвях, отвечающих `verification_required` (шаги 1а, 5 и 6), сервер той же
операцией ставит на запись состояние «нужна сверка» — но только на запись **без**
`yookassaPaymentId`, одинаково во всех трёх ветвях (расширено по находке MAJOR-D
состязательного ревью #112: решение владельца по MAJOR-5 было применено к шагу 1а, а на
шагах 5 и 6 пометка ставилась безусловно. `requestId` секретом не является — он копируемый,
уходит оператору платежей и виден в адресной строке, — поэтому посторонний, узнавший его,
портил чужое состояние: жертва видела «нужна сверка» вместо «оплата ещё идёт», и журнал
затапливался. Признак один и тот же во всех ветвях: запись с известным идентификатором
платежа разрешается проверкой статуса сама)** — `verificationAt` и
`verificationReason` (Решение 2, «Хранимое поле и кто его ставит») — и пишет журнальную
запись (Решение 2е). Разница в том, на какую запись: на шагах 5 и 6 — на найденную по
присланному `requestId`; на шаге 1а — на совпавшую по отпечатку каноническую, **и только если
у неё нет `yookassaPaymentId`**. Для
исправной канонической записи шаг 1а ответ даёт, а запись **не меняет** и в журнал не пишет —
иначе чужая двойная отправка портила бы состояние первого посетителя и засоряла бы журнал,
который является единственным сигналом института (решение владельца, MAJOR-5). Без этой
...
**Механика.** Первый `POST` с новым `requestId`, чей отпечаток совпал с подтверждённой записью
младше 14 суток, платежа не создаёт и получает `409 duplicate_confirmation_required`. Клиент
показывает вопрос «по этим данным оплата уже была подтверждена — создать ещё один платёж?».
Подтверждённый повтор идёт **тем же** новым `requestId` плюс `duplicateConfirmed: true`.
...
**Границы.** `canceled` подтверждения не требует — денег по такой записи уже не будет. После
14 суток с создания подтверждённой записи правило не применяется: срок тот же, что у блокировки
по отпечатку, и по той же причине — бессрочное предупреждение сделало бы повторную оплату того
же семинара невозможной навсегда.
...
Браузер удерживает **несколько** активных попыток: постоянно по каждой — только `requestId` и
время создания, значения полей — в хранилище на одну сессию. Новая попытка прежнего удержания
не снимает и не перезаписывает; интерфейс предупреждает, что предыдущая осталась
незавершённой.
...
**новой** попытки, то есть нового `requestId` (спека, «Незавершённая попытка не теряется при
...
несёт элемента, создающего **новую** попытку (элемент продолжения этой же попытки, наоборот,
обязателен — вариант B). Прежняя формулировка через отрицание («не форма») была зелёной на
...
**Оснований ровно три, и четвёртого в этом change нет.** Ручного снятия удержания
...
терминальное состояние человеком в контракте нет**. Прежняя редакция этого абзаца утверждала
...
платежей, `not_found`, либо 14 суток), её ничто не разрешает. **Нового или скрытого
...
6. **Архивирование разблокировано, реализация — нет.** Известно, чему требования дельта,
   поэтому запрет `/opsx:archive` из Ограничения объёма №5 снят. Остаётся обычный Definition of
