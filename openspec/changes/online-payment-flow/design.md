...
- **`PAYMENT_MODE=demo`**: процесс не проверяет `RECEIPT_ENABLED`, `HMAC_KEY_*` и
  ...
  создаёт записей идемпотентности (Requirement «CI/preview не создаёт платежей, а
  развёрнутый стенд работает с тестовым магазином»), поэтому эти секреты ему не нужны.
...
же формате, что установленный (Requirement «CI/preview не создаёт платежей, а
развёрнутый стенд работает с тестовым магазином») — его ответ на `POST .../payments`
**всегда** `200 { "status":
...
- **Механизм и единый наблюдаемый исход — Решение 1в, не повторяется здесь.**
  ...
  имеет (Requirement «CI/preview не создаёт платежей, а развёрнутый стенд работает с
  тестовым магазином»). В `PAYMENT_MODE=test` она обязательна наравне с production,
