...
| `503` | `{ "status": "verification_required" }` | статус записи не терминален (`pending`/`unknown`), и либо версия ключа не входит в число хранимых, либо содержимое не совпало (Решение 2а, шаги 5–6; Решение 3б, «Третий исход») — не `409`, `requestId` не заменяется автоматически. **Тот же ответ на новый, неизвестный серверу `requestId`, отпечаток которого совпал с незавершённой записью** (Решение 2е, шаг 1а «Итогового порядка») |
...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   ...
   ключа которой известна серверу, — ответ `verification_required`, платёж не создаётся.
...
5. Если запись найдена, её статус **не терминален** (`pending` или `unknown`), и
   ...
   записи нет выхода вовсе — состояние существующего здесь пусто). **Несовпадение → НЕ
   `409`, а
...
по наличию идентификатора платежа ЮKassa у записи. Ось не отменяет разбор выше, а
уточняет его: она относится не к содержимому запроса, а к тому, что успел записать сам
сервер.

| Статус | Версия | Содержимое | `paymentId` | Ветвь | Ответ | Новый `requestId` разрешён? |
|---|---|---|---|---|---|---|
| нетерминальный | известна | совпало | есть | 5 | состояние существующего | **нет** |
| нетерминальный | известна | совпало | нет | 5 | повтор к ЮKassa с тем же ключом идемпотентности, ответ по результату | **нет** |
...
`requestId`. Для записи, у которой есть идентификатор платежа ЮKassa, это не тупик:
проверка статуса разрешает её в терминальное состояние, и удержание снимается само.
Тупик остаётся ровно у записи **без** идентификатора платежа — созданной до вызова
ЮKassa (Решение 2б) и не получившей ответа. Ей и посвящён следующий абзац.
...
- **Кто и на каком основании снимает удержание после ручной сверки?** Записи, которых
  ...
  **(3)** каким действием удержание снимается — операторской командой на сервисе,
  указанием посетителю, или никак (ждём TTL). Пока ответа нет, действует TTL записи
