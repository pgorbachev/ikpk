...
**Чек (54-ФЗ) — определён нормативно сейчас, не отложен (решение владельца, Развилка 3
`proposal.md`).** Запрос создания платежа передаёт объект `receipt`:
`receipt.customer.email` — значение поля `email` формы (нужно ЮKassa, чтобы отправить
чек покупателю, если автогенерация включена на мерчант-аккаунте); `receipt.items` —
один элемент с `description` (то же значение, что у `seminar`, обрезанное до 128
символов — ограничение поля у ЮKassa), `quantity: "1.00"`, `amount.value` равным
`amount` запроса, `amount.currency: "RUB"`, `vat_code`, `payment_subject: "service"`,
`payment_mode: "full_payment"`. **Точное значение `vat_code`** (institution — образовательная
организация, вероятная льгота по НДС) подтверждается по тестовому магазину до
написания красных тестов (`tasks.md`, 2.2a) — это единственное числовое значение в
объекте `receipt`, не определённое окончательно в этой редакции; структура объекта
определена полностью и не меняется в зависимости от ответа. Если мерчант-аккаунт
ЮKassa не использует автогенерацию чеков вовсе, поле `receipt` передаётся, но ЮKassa
его игнорирует — контракт не ветвится по этому условию.
