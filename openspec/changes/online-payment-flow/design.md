...
не проверял бы до боевого запуска.
...
**Разблокировка после ручной сверки — механизм решён (Решение 2ж), уполномоченный и
критерий заключения остаются открытыми.** Записи, которые автоматический выход не разрешает
...
Open Questions. До ответа на них команда существует и работает, но кто её вправе вызвать —
вопрос эксплуатации, а не контракта; параллельно действует TTL записи (14 суток, Решение 4),
после которого тот же состав данных снова принимается.
...
- **Серверный контроль по отпечатку «занимает» состав данных — но не дольше 14 суток
  ...
  уполномочен и чем подтверждает заключение.
...
- **Кто уполномочен снимать удержание после ручной сверки и чем подтверждается его
  ...
  требуют человека. **Каким действием** удержание снимается — решено владельцем 2026-08-13:
  операторская команда сервиса, обязательная причина, запись в журнал (Решение 2ж); этот
  подвопрос закрыт и здесь больше не стоит. Остаются два, и оба вне репозитория:
  ...
  **(2)** чем это заключение подтверждается — записью в панели, её отсутствием, выпиской:
  ...
  какой редакции. Пока ответа нет, помимо команды действует TTL записи
