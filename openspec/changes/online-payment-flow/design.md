...
две ротации подряд), SHALL трактоваться как несовпадение отпечатка — тот же `409`, что
и при подмене содержимого. Для таких записей отдельная сознательная отправка с новым
`requestId` (Решение 2а) остаётся доступна; теряется только «бесплатная» идемпотентность
конкретного зависшего повтора, а не платёж как таковой.
