...
| установленный `stand` | форма обязательна, адрес стендового API обязателен | `test`, полный контракт создания и статуса | тестовый `1440249` |
| установленный `prod` | форма обязательна, `https://api.ikpk.su` обязателен | `prod`, полный контракт создания и статуса | боевой `409285` |

Workflow GitHub Pages `deploy.yml` в первую строку не входит: это production-деплой и
он обязан выбрать `prod`. `test` отличается от `prod` не формой HTTP-контракта и не клиентским UX, а магазином и
