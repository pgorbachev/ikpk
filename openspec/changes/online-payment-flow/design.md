...
идентификатору, а предупреждение обязательно для удержанных состояний «оплата ещё идёт» и
«статус пока неизвестен». Пять исключений, где предупреждения нет: отправка (предупреждение
преждевременно); отклонение по проверке, лимит частоты, демо, вопрос о повторной оплате
(в последних четырёх ответ доказывает отсутствие платежа). Заодно перестаёт засоряться журнал: в него попадали обычные двойные отправки, и
...
| `409 duplicate_confirmation_required` | «по этим данным оплата уже подтверждена — создать ещё один платёж?» с явным подтверждением; при подтверждении повтор идёт с тем же новым `requestId`, `duplicateConfirmed: true` **и `confirmationToken` из этого ответа**; без действительного токена запрос не отправляется (Решение 2к; задача 5.5f) |
