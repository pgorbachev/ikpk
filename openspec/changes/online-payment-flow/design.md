...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   ...
   **незавершённые** записи (живая блокировка по отпечатку); только если совпадения среди них
   нет — **подтверждённые не ранее 14 суток назад, известной версии ключа** (отпечаток
...
`yookassaPaymentId` **и только когда повод относится к самой записи** (неизвестная версия её
ключа — шаг 6; совпадение её отпечатка с запросом под другим `requestId` — шаг 1а). **Шаг 5
(несовпадение присланного содержимого) пометки и журнальной записи не делает вовсе**: это
свойство запроса, а не записи, и им управляет посторонний (MAJOR-D состязательного ревью
`r5-adv`). Во всех остальных отношениях правило одинаково во всех трёх ветвях (расширено по находке MAJOR-D
...
несёт элемента, создающего новую попытку **вместо** удерживаемой. Элемент продолжения этой же
попытки, наоборот, обязателен (вариант B), и явное действие «оплатить другой семинар»
...
(исправлено по MAJOR-4 ревью r5: прежняя редакция говорила «каждый ответ» и противоречила
правилу MAJOR-5 двумя абзацами выше — исправная запись не помечается и в журнал не пишется) (неизвестная версия ключа / несовпадение содержимого /
