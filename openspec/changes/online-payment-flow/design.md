...
- **прод-контур раздваивается, и это существенно** (найдено ревью PR #90, B2, разбор — в
  ...
  уже сегодня и **неверно для прода**. Развилка 1 решена (Pages плюс `api.ikpk.su`), а затем
  **заменена решением владельца от 2026-08-18**: Pages платёжной поверхностью не является,
  `api.ikpk.su` не поднимается, и одинаковый origin у страницы и API становится нормой
  стенда, а не совпадением. Каждое решение ниже, зависящее от этого, отмечено явно;
...
**Решено владельцем: минимальный сервис на VPS**, отдельный от Strapi. **Решение по
Развилке 1 (`proposal.md`, B2) — «прод-API на `https://api.ikpk.su`, сайт на GitHub Pages» —
заменено решением владельца от 2026-08-18:** GitHub Pages платёжной поверхностью не
является, `api.ikpk.su` на этой фазе не поднимается, и первым (а пока единственным)
...
зелёной при полностью неработающем CORS на проде. Приёмка настоящим браузером против развёрнутого боевого API — обязательный отдельный шаг
(`tasks.md`, 7.3a), не покрываемый ни контрактными тестами, ни демо-приёмкой. **Отнесён к
прод-фазе решением владельца от 2026-08-18:** настоящей пары origin в текущей фазе не
существует вовсе — стенд раздаёт API на своём origin, прод-контур не разворачивается.
...
артефакт содержит форму и канонический `https://api.ikpk.su`, поэтому прежний build-гейт
зелёный, хотя API не развёрнут. Проверять только строку адреса недостаточно.
...
| установленный `prod` | форма обязательна, `https://api.ikpk.su` обязателен | `prod`, полный контракт создания и статуса | боевой `409285` |
...
относила Pages к `prod` и делала работающий `api.ikpk.su` предусловием слияния — эта
...
5. Выкатка: демо-стенд с тестовым магазином `1440249` первым; это полный тестовый путь,
   а не mock-режим. Боевая сборка — только после того, как HTTPS для `api.ikpk.su`,
   CORS и сервис с боевым магазином `409285` готовы. Для обоих контуров до публикации
   проходит readiness-проверка режима и `shopId` (Решение 13); отсутствие формы либо
   API блокирует деплой, а не отключает оплату.
