...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   ...
   **незавершённые** записи (живая блокировка по отпечатку); только если совпадения среди них
   нет — **подтверждённые** младше 14 суток: при совпадении и отсутствии действительного
   одноразового токена подтверждения ответ `409 duplicate_confirmation_required`, платёж не
   ...
   **но поиск по отпечатку выполняется заново и неделимо с созданием**, потому что между
   вопросом и подтверждением могла появиться незавершённая попытка того же состава, и она
   снова имеет приоритет. Прежний порядок (сначала `succeeded`, создание сразу по клиентскому
   флагу) при одновременных подтверждённой и незавершённой записях одного состава давал
   **третий** платёж. Итак, незавершённые: если
...
«`requestId` + отпечаток», с коротким сроком жизни, и запоминает у себя, что подтверждение по
этой паре запрошено. Подтверждённый повтор идёт **тем же** новым `requestId` и несёт этот
