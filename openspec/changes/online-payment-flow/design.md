...
Решением 13.** Для текста страницы различаются: CI без формы, CI/preview с mock-формой и
...
|---|---|---|---|
| обязательный workflow `Tests` / локальный preview | форма может отсутствовать; для клиентских сценариев допустима форма с mock-ответом | mock/`demo`, без сети к ЮKassa | отсутствует |
...
`ci|stand|prod`; устанавливаемый сервер — явную роль `test|prod`; отдельный `demo`
разрешён только CI/preview-процессу и не проходит deploy-гейт. Неизвестное значение — ошибка. Отсутствие
значения допускается только обычной CI-сборкой и означает отсутствие активной формы;
оно не является умолчанием для `stand` или `prod`.
...
2. объявленный эндпоинт доступен и отвечает ожидаемым машинным контрактом. **Схема не
   предписывается:** для нынешнего стенда это HTTP (`http://193.124.115.99`), требование
   HTTPS относится к прод-контуру (решение владельца от 2026-08-18);
...
4. для production отдельно проходит CORS-проверка с origin боевого сайта. Для стенда её нет:
   запрос same-origin, и браузер CORS к нему не применяет.
