...
если эта запись сама не разрешается автоматически**: нет `yookassaPaymentId` либо статус не
...
1а. **Если запись по `requestId` не найдена — прежде обычного создания сервер ищет
   ...
   **каноническим `requestId` этой совпавшей записи** (Решение 2е, «Канонический
   ...
   поле и кто его ставит») и пишет журнальную запись **только если эта запись сама не
   разрешается автоматически — то есть у неё нет `yookassaPaymentId` (или её статус не
   определён); исправную запись с известным `yookassaPaymentId` чужой запрос не ухудшает**
...
она сама не разрешается автоматически** (нет `yookassaPaymentId` либо статус не определён). Для
