...
- **прод-контур раздваивается, и это существенно** (найдено ревью PR #90, B2, разбор — в
  ...
  **заменена решением владельца от 2026-08-18**: Pages платёжной поверхностью не является,
  ...
  стенда, а не совпадением; Каждое
...
которого API раздаётся на **том же origin**, что и сам стенд, по префиксу `/api/payments`.
...
| установленный `stand` | форма обязательна, адрес — `<origin стенда>/api/payments` | `test`, полный контракт создания и статуса | тестовый `1440249` |
...
процессы), порты, env-файлы и каталоги состояния. Правило `root:root 0600` Решения 1
...
1. в артефакте есть ровно одна активная платёжная форма с адресом выбранного контура;
2. HTTPS-эндпоинт доступен и отвечает ожидаемым машинным контрактом;
3. readiness сообщает `test/1440249` для `stand` либо `prod/409285` для production;
4. для production отдельно проходит CORS-проверка с `Origin: https://ikpk.su`.
