...
- **Незавершённая попытка переживает перезагрузку страницы** — **внесено решением
  ...
  названным основаниям (включая демо-ответ проверки статуса); не ставится в демонстрационной сборке.
...
- **Состояние «нужна сверка» становится требованием, а не картинкой** — предупреждение
  ...
  **Дополнено вторым раундом ревью #112:** предупреждение и запрет подменять удерживаемую попытку
  требуются от состояний по действующему удержанию, **кроме** состояний, доказывающих отсутствие
  платежа (отправка, отклонение по проверке, лимит частоты, демо, вопрос о повторной оплате);
