...
- **Незавершённая попытка переживает перезагрузку страницы** — **внесено решением
  ...
  ревью #112:** удержание хранит не только идентификатор, но и время создания попытки и
  сообщённый исход (иначе после `F5` показывалось «статус неизвестен» с активной кнопкой);
  ставится в момент отправки, а не по ответу; снимается по трём названным основаниям;
  не ставится ни в демонстрационной сборке, ни по отказу, за которым нет серверной записи.
- **Состояние «нужна сверка» становится требованием, а не картинкой** — предупреждение
  ...
  **Дополнено вторым раундом ревью #112:** предупреждение и отсутствие отправки требуются
  ...
  фокус переходит в панель исхода и исход объявляется вспомогательной технологии, а
  идентификатор не показывается как код платежа там, где платёж лежит под другим
  идентификатором.
