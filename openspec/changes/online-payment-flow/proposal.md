...
- **Выход из состояния «нужна сверка» вручную — решением владельца от 2026-08-13** это
  ...
  **ОТЛОЖЕНО тем же днём более новым решением владельца (`design.md`, Решение 2з;
  ...
  текущей связи сайта, Bitrix и ЮKassa.
  **Понижено в статусе решением владельца от 2026-08-13 после ответа заказчика: командная
  строка для менеджеров принятым решением не считается** (`design.md`, «Архитектурные
  альтернативы», статус `Proposed / Not accepted`). Отсрочка сохранена и её основание
  изменилось: ответ заказчика пришёл, но операторский workflow всё равно не проектируется —
  это прямое указание владельца, а не ожидание письма. То есть в этот перечень изменений
  пункт входит **не как поставка и не как выбранное направление**, а как отклонённый на
  сегодня вариант; открытыми остаются кто уполномочен и чем подтверждает заключение.
...
6. **Настройки ЮKassa, не видимые из браузера, — внешний prerequisite production, но не
   ...
   Проверяется всё это на тестовом магазине (`tasks.md`, 2.2a) и подтверждается письменно
   (2.9) — **до** написания красных тестов раздела 3 там, где от этого зависит контракт (то
   же правило, что закрыло Развилку 3).
...
внутри того же аккаунта, API v3, webhook; Certbot/Let's Encrypt для HTTPS стенда.
