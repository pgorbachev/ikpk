...
**Дополнено 2026-08-13:** помимо описанного здесь порядка merge, у change появился внешний
prerequisite — ответ заказчика о текущей связи сайта, Bitrix и ЮKassa. Он блокирует
production-контракт и архивирование, но не контур ЮKassa и не демо: см. «Ограничения объёма»,
точка 5, и `design.md`, Решение 2з.
...
- **Незавершённая попытка переживает перезагрузку страницы** — **внесено решением
  ...
  ревью #112:** удержание ставится в момент отправки, а не по ответу; снимается по трём
  названным основаниям; не ставится в демонстрационной сборке.
...
- **Состояние «нужна сверка» становится требованием, а не картинкой** — предупреждение
  «платёж мог быть создан, повторять не нужно», отсутствие элемента отправки и
  копируемый идентификатор попытки прежде несли только мокапы, то есть ничто.
  **Дополнено вторым раундом ревью #112:** предупреждение и отсутствие отправки требуются
  от **всех** состояний по действующему удержанию, исход отличается машинным признаком,
  фокус переходит в панель исхода и исход объявляется вспомогательной технологии.
  **Дополнено решением владельца от 2026-08-13:** при совпадении отпечатка сервер возвращает
  **канонический** `requestId` исходной попытки, клиент переключается на него и удерживает
  попытку по нему — поэтому показываемый идентификатор всегда тот, под которым платёж лежит у
  оператора платежей, и прежней ветви «идентификатор не показывать» больше нет. Цена в
  приватности названа отклонением в `design.md`, Решение 2е.
- **Выход из состояния «нужна сверка» вручную — решением владельца от 2026-08-13** это
  операторская команда сервиса: обязательная причина, запись в журнал, отказ при вызове без
  причины; административная панель отвергнута как более сложная и менее безопасная
  (`design.md`, Решение 2ж). Кто уполномочен и чем подтверждает заключение — остаётся
  открытым вопросом: внешняя роль и операционный критерий, из репозитория не выводимые.
  **ОТЛОЖЕНО тем же днём более новым решением владельца (`design.md`, Решение 2з;
  Ограничение объёма №5):** операторский workflow не проектируется до ответа заказчика о
  текущей связи сайта, Bitrix и ЮKassa. Решение остаётся записанным как выбранное
  направление, реализация по нему не начинается, и в этот перечень изменений оно на сегодня
  входит именно в таком статусе — не как поставка.
...
- **Границы суммы и лимит частоты создания платежа** — диапазон суммы (нижняя и
  верхняя граница, `design.md`, Решение 6) защищает от ошибки ввода и от card-testing;
  лимит частоты (Решение 5б) нужен в первую очередь для **боевого** контура, где
  учётные данные и деньги настоящие — стенд с тестовыми учётными данными ЮKassa лишь
  первое окружение, где отсутствие лимита стало заметно при разборе объёма, не
  причина требования. **Исправлено по находке ревью #97 (minor):** первая редакция
  этого пункта называла только верхнюю границу суммы и объясняла лимит частоты
...
суррогатом. **Точка 1 закрыта решением владельца 2026-08-12; открытыми остаются точки 2–5.**
...
читалась как то, что кто-то потерял из виду.
...
1. **Рантайм секрета — РЕШЕНО (владелец, 2026-08-12).** Сайт статический; секрет ЮKassa
   ...
   на существующей VPS, ровно одна реплика, секрет — файл окружения, принадлежащий
   отдельному системному пользователю сервиса** (`design.md`, Решение 1, Решение 1б,
   Решение 7). Вариант «отдельная VPS только под платежи» **отложен, а не отвергнут** —
...
3. **Webhook требует публичного HTTPS.** Стенд сейчас поднимается `bootstrap-vps.sh` с
   ...
   документу). **Отдельно от стенда** (решено по Развилке 1): прод-API
...
5. **Ответ заказчика о текущей связи сайта, Bitrix и ЮKassa — внешний prerequisite
   production-контракта и архивирования. Внесено решением владельца от 2026-08-13.** У
   заказчика запрошены скриншоты: заявка до оплаты, заявка после оплаты, платёж в панели
   ЮKassa, сообщение, которое получает клиент. Ожидаемый срок ответа — 1–2 дня от
   2026-08-13. Перечень того, что именно неизвестно (пять пунктов), и почему без ответа
   контракт не определить — `design.md`, Решение 2з.

   **Что этой точкой заблокировано:**
   - **production-контракт** этого change: боевой путь оплаты не выкладывается и
     существующий бизнес-процесс не меняется без согласования с заказчиком;
   - **архивирование** change (`tasks.md`, 7.8): требования не переезжают в
     `openspec/specs/`, пока не известно, чему они дельта;
   - **операторский workflow** — ручное снятие удержания (`design.md`, Решение 2ж):
     решение сохранено как выбранное направление, реализация по нему не начинается. Вместе
     с ним пока не проектируются персональные ссылки на оплату, новая интеграция с Bitrix и
     отдельная административная панель (последняя владельцем уже отвергнута по существу).

   **Что этой точкой не заблокировано:** контур ЮKassa (точки 2 и 3 выше — регистрация
   тестового магазина, наблюдения на нём, HTTPS, mock- и контрактные тесты серверного
   эндпоинта) и демонстрационная сборка. Это прямо разрешено тем же решением владельца:
   ни боевых денег, ни процесса института они не касаются.

Итог: change закрывается либо полной реализацией и приёмкой, либо честным `blocked
(external)` на конкретной из точек **2–3 и 5** (точка 1 закрыта решением владельца
2026-08-12, `blocked` на ней больше невозможен) без суррогатных зелёных проверок — в духе
`Definition of Done итерации` плана `witty-wiggling-fountain.md`, п. 1. Точка 5 отличается
от 2–3 тем, что она блокирует **не только приёмку, но и архивирование**: на ней change не
может быть закрыт даже как `blocked (external)` со ссылкой «остальное сделано» — пока ответа
нет, требования в `openspec/specs/` не переезжают.
...
- секрет живёт на новом рантайме, которого в проекте раньше не было — новая
  ...
  (`scripts/deploy-web.sh:50-52`). Принятая мера — отдельный системный пользователь
  сервиса (`design.md`, Решение 1); полная изоляция — отложенный вариант «отдельная VPS»
