...
Workflow публикации GitHub Pages (`deploy.yml`) к этому исключению **не относится**:
он выпускает production и обязан использовать ветвь `prod` со включённой формой либо
завершиться отказом до публикации.

**Объём отложен решением владельца от 2026-08-18: матрица вызова API изъята из PR #112.**
Всё, что опирается на живой платёжный API — предусловие слияния по `api.ikpk.su`, гейт
публикации по readiness, адрес стендового API и остановка выкладки при недоступности, —
**в PR #112 не входит** и остаётся отдельной работой на этой ветке. Причина: 19.08.2026
состоится разговор с заказчиком, и до него решение о вызове API не принимается. До тех пор
реализуется всё, что от этого API не зависит.

Практическое следствие, названное честно: пока матрица не внесена, боевая сборка
по-прежнему несёт форму с адресом `https://api.ikpk.su`, которого не существует, поэтому
**PR #112 не сливается** — не из-за неготовности кода, а из-за нерешённого объёма. Это не
регресс и не новая находка, а то же самое препятствие, только теперь названное решением, а
не оставленное молчанием.

**Цена решения названа и принята (M1, решение владельца от 2026-08-18).** Раз production не
имеет ветви выключения формы, а гейт публикации требует доступного API, то до появления
`https://api.ikpk.su` выкладка **нового** Pages-артефакта останавливается целиком — включая
изменения, к оплате не относящиеся. Уже опубликованный сайт не исчезает: остаётся последняя
успешная версия. Следствие оформлено явно: работающий `api.ikpk.su` — предусловие **слияния**
PR #112 (`tasks.md`, 2.3b), а не задача «к прому». Адрес стенда — `https://api.staging.ikpk.su`
(`tasks.md`, 2.3a), и у него предусловиями остаются сертификат и сама служба.
...
`1440249`, закреплённый за демо-стендом, API v3, webhook; Certbot/Let's Encrypt для HTTPS
стенда. CI/preview не получает секретов ни одного из магазинов.
