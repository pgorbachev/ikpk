...
архивированы). `payment-request` существует только как делта внутри неархивированного
`payment-request-flow` и не является `openspec/specs/`; формально это не `Modified
Capabilities` в терминах процесса. По существу же `online-payment-flow` меняет судьбу
`payment-request`, что и описано выше в разделе «Отношение к `payment-request-flow`» —
эта зависимость называется прямо, а не через формальный делта-механизм, которого
процесс для двух параллельных, ещё не заархивированных change не предусматривает.
