...
- **Состояние «нужна сверка» становится требованием, а не картинкой** — предупреждение
  ...
  попытку, и копируемый идентификатор попытки прежде несли только мокапы, то есть ничто.
  **Формулировка уточнена 2026-08-13 (вариант B):** запрещено создание новой попытки, а элемент
  продолжения этой же обязателен — прежнее «отсутствие элемента отправки» противоречило бы
  безопасному продолжению.
  **Дополнено вторым раундом ревью #112:** предупреждение и запрет на создание новой попытки
  требуются от **всех** состояний по действующему удержанию, исход отличается машинным признаком,
...
- **Ручного выхода из состояния «нужна сверка» в change НЕТ, и это решение владельца от
  ...
  снимается только тремя автоматическими основаниями: терминальный статус от оператора платежей,
  «записи нет», 14 суток. Открытыми остаются кто уполномочен заключить, что платежа не было, и
...
- **Повторная оплата того же состава после подтверждённой — вопрос, а не запрет (решение
  ...
  подтверждённой оплатой младше 14 суток, платежа не создаёт: посетителя спрашивают, создавать
...
- **Продолжение незавершённой попытки — решение владельца от 2026-08-13 (вариант B).** В
  ...
  только создание новой попытки с новым `requestId`. Это единственный исполнитель безопасного
