...
**Продолжение этой истории — решение владельца 2026-08-20/21, см. «Capabilities» выше.**
Копирование `request-form-transport` в этот change (P1-4, ниже) решало задачу момента —
дать требованию дом в `openspec/specs/` при отсутствии его настоящего источника — но
оставило после себя копию описания уже существующего сайта, выданную за работу этого
change. Решение снято: возможность удалена отсюда, её требования, нужные диалогу оплаты,
самостоятельно сформулированы в `specs/online-payment/spec.md`, а её настоящее место —
...
### Развилка 1 (B2): на каком origin живёт прод-эндпоинт — рассмотрена, решение вынесено в `production-payment-rollout`

**Пересмотрено решением владельца 2026-08-20/21: `https://api.ikpk.su` больше не
принятое production-решение этого change.** Ниже сохранён разбор 2026-08-11 как
исследованные варианты — оба остаются непринятыми альтернативами, ни один не выбран.
Production endpoint и схема размещения **не определены**: выбор — предмет отдельного
будущего change `production-payment-rollout` (см. `tasks.md`, «Future work»), не этого.
Формулировка «Решено владельцем — вариант 1» ниже верна как историческая запись momента
написания (2026-08-11) и неверна как описание текущего состояния — решение отложено,
а не отменено содержательно: причины, по которым вариант 1 выглядел предпочтительным,
не изменились, изменилось только то, что этот change его не принимает сам.
...
**Записано 2026-08-11 как решение владельца — вариант 1: `https://api.ikpk.su` на VPS,
сайт остаётся на GitHub Pages.** Меньше побочных эффектов: не трогает уже работающий и
проверенный (`AGENTS.md`, «Гейт публикации») конвейер Pages. **Пересмотрено 2026-08-20/21:
это была верная запись на момент написания, а не действующее решение сегодня** — выбор
между вариантом 1 и вариантом 2 (или иным, здесь не рассмотренным) относится к объёму
`production-payment-rollout`, а не к этому change. CORS-политика (allowlist ровно на
origin боевого сайта, preflight-обработка) остаётся нормативным требованием для ЛЮБОГО
production endpoint, который тот change выберет (см. `design.md`, Решение 1а, и
`specs/online-payment/spec.md`, Requirement «CORS ограничен доменом сайта») — она не
привязана буквально к `api.ikpk.su`.
...
собрать страницу без оплаты существует только для обязательного workflow `Tests` и Pages —
это роль `ci`; локальный preview и клиентские сценарии собираются в роли `preview` с
mock-формой (решение владельца от 2026-08-19). Такой артефакт обязан быть объективно отличим и не
...
оплату на странице, а роли сборки ещё нет. Всё сказанное про роли `ci` и `preview` — требование к
...
без платёжной формы и без обещания оплаты на странице, поэтому ветвь `prod` гейта к нему не
...
- **входит в работу этой ветки:** демо-стенд с **тестовым магазином `1440249`** — полный
  ...
  `ci|preview|stand|prod` (роль `preview` выделена из `ci` решением владельца от 2026-08-19),
  readiness-ответ, безопасная проба `OPTIONS` публичного пути, гейт выкладки стенда;
...
- **не входит:** прод-контур целиком (решение владельца от 2026-08-18, расширено
  2026-08-20/21) — адрес и топология production API не выбраны этим change (см.
  «Развилка 1» выше), поэтому ни его подъём, ни его HTTPS, ни его CORS, ни боевой
  магазин `409285` в текущую фазу не входят; это объём `production-payment-rollout`.
...
- `online-payment`: создание платежа за семинар на `/oplata`, редирект в ЮKassa,
  ...
  стенда, требования к обращению с секретом, доступность и управление диалогом оплаты,
  согласие на ПДн, защита от автоматических отправок и поведение без скриптов
  (перенесено из `request-form-transport` решением владельца 2026-08-20/21, см. ниже).

**`request-form-transport` больше не входит в этот change (решение владельца
2026-08-20/21).** Прежняя редакция объявляла эту возможность собственной делтой
`online-payment-flow` (см. историю ниже, «Исправлено по находке ревью P1-4») — на
практике это была копия описания уже существующего на сайте общего механизма
форм-заявок, скопированная из удалённого `payment-request-flow`, а не работа,
выполняемая этим change. Правильное место для неё — будущий baseline-change по
существующему транспорту заявки, которого пока нет в `openspec/changes/`. Требования,
которые этот механизм давал диалогу оплаты (модальность, доступность полей, согласие на
ПДн, поведение без скриптов, защита от автоматических отправок, клиентская блокировка
повторной отправки), перенесены в `specs/online-payment/spec.md` как собственные
требования к диалогу оплаты — они не исчезают, а формулируются самостоятельно, без
зависимости от возможности, которой в `openspec/specs/` не будет до отдельной будущей
работы. Файл `specs/request-form-transport/spec.md` удалён из этого change тем же
решением.

### Modified Capabilities

- `deploy-gating` (существующая базовая возможность в `openspec/specs/deploy-gating/`):
  Requirement «Проверки демо-режима входят в обязательный прогон» дополнен ролью
  артефакта как предметом проверки — роль `ci` без платёжной формы не должна читаться
  как «непройденная проверка» на артефакте, который по контракту роли формы не несёт
  (`specs/deploy-gating/spec.md`, найдено M7 независимого ревью спеки 2026-08-18,
  уточнено D1 сессии красных тестов 2026-08-19). **Раздел не называл эту дельту
  явно — исправлено при `/opsx:update` 2026-08-20/21**, при подготовке архивирования:
  до этого исправления раздел ошибочно утверждал «Modified Capabilities: нет», хотя
  файл `specs/deploy-gating/spec.md` с `MODIFIED Requirements` существовал в этом
  change с 2026-08-18/19.

`article-catalog` (**исправлено, P2-C, факт устарел**): первая редакция утверждала, что
...
эту возможность не касается, поэтому `article-catalog` в `Modified Capabilities` не
входит — не потому, что каталог пуст, а потому что области не пересекаются. Возможность
...
самостоятельная работа, которой в `openspec/changes/` пока нет — зафиксирована как будущая
задача `baseline-request-form-transport` (решение владельца 2026-08-20/21). Прежняя
редакция этого раздела делала её появление предусловием архивирования `online-payment-flow`
(запрет преждевременного архива при пересечении областей, `AGENTS.md`, раздел про baseline,
пункты 7–8) — это больше не так: пересечения не осталось, потому что возможность
`request-form-transport` из этого change удалена, а не архивируется вместе с ним.
