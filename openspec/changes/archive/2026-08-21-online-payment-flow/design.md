...
`payment-request-flow` замещён. Его требование `request-form-transport` было скопировано в
`specs/request-form-transport/` этого change, а затем решением владельца 2026-08-20/21
удалено оттуда: та копия описывала уже существующий на сайте механизм, не работу этого
change, и её настоящее место — будущий `baseline-request-form-transport`. То, что из неё
было специфично для диалога оплаты, сформулировано как собственные требования в
`specs/online-payment/spec.md` (Requirements про модальность, доступность полей,
согласие на ПДн, блокировку повторной отправки, поведение без скриптов и защиту от
автоматических отправок). Многочисленные ссылки на
...
- диалог оплаты несёт собственные требования к модальности, доступности, согласию на
  ПДн и поведению без скриптов (`specs/online-payment/spec.md`) — до 2026-08-20/21 они
  жили в отдельной возможности `request-form-transport`, скопированной из уже дважды
  отревьюенной делты `payment-request-flow` (P1-4а); решением владельца та возможность
  удалена из этого change как описывающая уже существующий сайт, а не его работу, и
  специфичная для оплаты часть перенесена сюда самостоятельными требованиями;
...
- **прод-контур раздваивается, и это существенно** (найдено ревью PR #90, B2, разбор — в
  ...
  уже сегодня и **неверно для прода**. Развилка 1 рассмотрена (Pages плюс `api.ikpk.su`
  как один из вариантов), но не принята как решение этого change (пересмотрено
  2026-08-20/21 — выбор production endpoint отложен до `production-payment-rollout`).
  **Решением владельца от 2026-08-18** отдельно установлено: Pages платёжной
  поверхностью не является, прод-контур в этой фазе не разворачивается ни по какому
  адресу, и одинаковый origin у страницы и API — норма стенда, а не совпадение. Каждое
...
- диалог оплаты доступен и безопасен по тем же меркам, что и любая форма-заявка сайта
  (модальность, доступность, согласие на ПДн, защита от ботов) — требования к этому
  сформулированы самостоятельно в `specs/online-payment/spec.md`, без зависимости от
  возможности, удалённой из этого change 2026-08-20/21 (см. «Context» выше).
...
**Решено владельцем: минимальный сервис на VPS**, отдельный от Strapi. **Рассмотренный
вариант по Развилке 1 (`proposal.md`, B2) — «прод-API на `https://api.ikpk.su`, сайт на
GitHub Pages» — не принят этим change (пересмотрено 2026-08-20/21, выбор production
endpoint отложен до `production-payment-rollout`).** Решением владельца от 2026-08-18
установлено отдельно: GitHub Pages платёжной поверхностью не является, прод-контур в
этой фазе не разворачивается, и первым (а пока единственным)
устанавливаемым контуром становится **демо-стенд с тестовым магазином `1440249`**, у
которого API раздаётся на **том же origin**, что и сам стенд: объявляемая база —
`<origin стенда>/api`, а внешние платёжные пути — `/api/payments` и
`/api/payments/{requestId}/status`, потому что путь клиент дописывает сам.
Прежнее решение сохранено как история: оно объясняет, откуда в тексте кросс-origin CORS и
требование HTTPS на API-стороне — оба относятся к прод-контуру, а не к стенду.
...
отдельный сервис на существующей VPS, ровно одна реплика каждого установленного контура
(Решение 1б/13), секрет и ключ отпечатка — отдельные файлы окружения
`/etc/ikpk-payments/stand.env` и `/etc/ikpk-payments/prod.env`, владелец `root:root`,
права `0600`** (Решение 7/13, задача 4.5). Разбор — материал для решения, **не источник требований**: при
...
не меняется** — обе инстанции остаются `User=ikpk-payments`; **изменяемые данные сервиса**
(хранилище идемпотентности, журнал canary, резервные копии) принадлежат `ikpk-payments`
и живут отдельно от конфигурации и друг от друга (например,
`/var/lib/ikpk-payments/{stand,prod}`); **обычный деплой
...
Платёжный API **прод-контура** SHALL отвечать заголовками CORS, разрешающими запросы
**только** с origin боевого сайта (`https://ikpk.su`). **Уточнено решением владельца от
2026-08-18:** стенд из этого требования исключён — у него страница и API на одном origin,
поэтому браузер CORS к запросу не применяет, и никакого демо-поддомена в списке разрешённых
origin держать не нужно; захардкоженный один origin в коде правки ради стенда не требует.
...
кросс-origin между сайтом и его API, какой бы адрес тот в итоге не получил (Развилка 1,
не принята этим change); демо-стенд — не заменяет эту проверку, потому что там страница
и API на одном origin (Решение 1), и браузер вообще не применяет CORS к запросу в
пределах одного origin — приёмка на стенде остаётся зелёной при полностью неработающем
CORS на проде. Приёмка настоящим браузером против развёрнутого боевого API — отдельный
шаг вне объёма этого change (`tasks.md`, «Future work», бывший 7.3a), не покрываемый ни
контрактными тестами, ни демо-приёмкой. **Вынесено из объёма этой фазы решением
владельца от 2026-08-18, расширено 2026-08-20/21:** настоящей пары origin в текущей
фазе не существует вовсе — стенд раздаёт API на своём origin, прод-контур не
разворачивается ни по какому адресу.
...
Всё это верно **только при ровно одной работающей реплике каждого контура**. Две
изолированные инстанции `stand` и `prod` из Решения 13 — не реплики: у них разные
магазины, конфигурация и состояние. Схема, где внутри одного контура рядом запущены
две реплики (например, blue-green на том же VPS во время выкладки новой
...
репликой на контур**. Если операционная практика позже потребует нескольких (масштабирование,
...
### Решение 1в: `PAYMENT_MODE` — один переключатель роли, одна точка проверки обязательных секретов, один наблюдаемый исход

**Пересмотрено решением владельца 2026-08-18 (Решение 13).** Двоичное деление
`demo|prod` больше не описывает контуры выпуска: установленный демо-стенд не является
mock-режимом и обращается к тестовому магазину ЮKassa. Описание `PAYMENT_MODE=demo`
ниже сохраняет смысл только для CI/preview-обработчика без внешнего платежа; оно не
разрешает разворачивать такой процесс как `stand`. Для устанавливаемых контуров
нормативна матрица `test|prod` и проверка магазина из Решения 13.
...
- `PAYMENT_MODE=demo|test|prod` — обязательная переменная окружения серверного процесса,
...
- **Найдено ревью #97, MAJOR-6 (владелец): сам `PAYMENT_MODE` не был нормативен, а
  ...
  `{demo, test, prod}` (включая опечатку вида `production`, `Prod`) — процесс завершается
...
- **`PAYMENT_MODE=demo`**: процесс не проверяет `RECEIPT_ENABLED`, `HMAC_KEY_*` и
  ...
  создаёт записей идемпотентности (Requirement «Роли `ci` и `preview` не создают платежей, а развёрнутый стенд работает с тестовым магазином»), поэтому эти секреты ему не нужны.
  Этот режим не является устанавливаемым стендом и отвергается deploy-гейтом.
- **`PAYMENT_MODE=test|prod`**: при старте процесс проверяет **весь список** обязательных
  для платёжного режима секретов (секрет ЮKassa, `HMAC_KEY_CURRENT` и его версия,
  ...
  — Решение 3б). Кроме того, `test` требует `shopId=1440249`, а `prod` — `shopId=409285`;
  несовпадение завершает процесс до открытия порта (Решение 13). Список — не перечисление в тексте требования, а то, что реализация
...
**Рассогласование «установленная сборка × CI mock-сервер» (найдено ревью #97, MAJOR-6;
переформулировано Решением 13).** Сборка сайта и режим сервера, на который она указывает, конфигурируются
раздельно (`DEMO_FORMS`/адрес формы — на стороне сборки; `PAYMENT_MODE` — на стороне
сервера) — рассогласование не проверяется форматом запроса, потому что он одинаков.
Теперь его обязан ловить автоматический deploy-гейт по readiness (Решение 13, задачи
3.16 и 6.13), а не только ручная приёмка. CI/preview обработчик получает запрос в том
же формате, что установленный (Requirement «Роли `ci` и `preview` не создают платежей, а развёрнутый стенд работает с тестовым магазином») — его ответ на `POST .../payments`
**всегда** `200 { "status":
"created_demo" }` (добавлено в таблицу кодов ниже), независимо от того, ждёт ли его
установленный или CI-клиент. Установленный клиент, получивший `created_demo` вместо
...
|---|---|---|---|
| `requestId` | string | да | валидный UUID v4; формат проверяется — не-UUID отклоняется `400` до всякого обращения к ЮKassa |
...
| `400`, `422` | `{ "status": "rejected", "errors": [{ "field": "...", "message": "..." }] }` | отклонена по проверке, включая невалидный формат `requestId` |
...
| `200` | `{ "status": "created_demo" }` | ответ CI/preview mock-обработчика — всегда этот код и это тело (Решение 1в, область сужена Решением 13); установленный клиент трактует его как ошибку адресата. **Исход терминальный и демонстрационный:** записи он не создаёт, удержания не ставит, и следующая отправка CI/preview-формы формирует новый `requestId`. Развёрнутый стенд этот ответ возвращать не может: он работает с тестовым магазином `1440249` |
...
**Отображение на состояния формы** — общий конверт исходов создания (отклонено
проверкой / ошибка / нет ответа за 10 секунд / принято к обработке), плюс специфичные
для платежа, все описанные возможностью `online-payment` в спеке:
...
| `400`/`422 rejected` | отклонена по проверке |
...
| `429` | лимит частоты превышен («ошибка адресата», с уточнением текста) |
...
| ошибка / нет ответа за 10 секунд | ошибка адресата / исход неизвестен |
...
- **Механизм и единый наблюдаемый исход — Решение 1в, не повторяется здесь.**
  ...
  имеет (Requirement «Роли `ci` и `preview` не создают платежей, а развёрнутый стенд работает с тестовым магазином»). В `PAYMENT_MODE=test` она обязательна наравне с production,
  чтобы стенд проверял полный запрос к ЮKassa. Найдено
...
Первая редакция использовала «10 секунд» и для клиентского предела создания платежа, и
для клиентского предела опроса статуса, и
...
`confirmation.return_url` вида `<база возврата контура>/oplata?paymentRequest=<requestId>` —
адрес возврата, на который ЮKassa направит браузер после завершения оплаты.

**База возврата — свойство контура, и она обязана быть задана явно (решение владельца от
2026-08-18).** Для стенда это `http://193.124.115.99`, для прод-контура — origin боевого
сайта. Названо отдельным абзацем, потому что цена ошибки здесь наблюдается только вживую:
переменная `PAYMENT_RETURN_BASE` в коде имеет умолчание `https://ikpk.su`, и стенд, у
которого она не задана, отправил бы посетителя после **тестовой** оплаты на боевой сайт — где
нет ни его удержания, ни, по решению того же дня, самой формы. То есть путь, ради которого
стенд и строится, обрывался бы молча на последнем шаге, а гейт выкладки этого не видит: он
проверяет артефакт и readiness, а не адрес возврата, который сервер подставит потом. Поэтому
база возврата входит в env-файл контура наравне с `shopId` и ключом.

**Исход при незаданной базе — отказ до открытия порта (решение владельца от 2026-08-19,
находка A1).** Для режимов `test` и `prod` `PAYMENT_RETURN_BASE` входит в тот же обязательный
набор, что учётные данные и ключ отпечатка, и её отсутствие даёт то же единственное
наблюдаемое: сервис не начинает обработку запросов. Вариант «запрещено конфигурацией, но
процесс стартует» отвергнут: тогда ошибка настройки обнаруживается только вживую, на
последнем шаге оплаты, и не оператором, а посетителем.
...
предупреждение и способ связаться напрямую от неё не зависят. Само
...
нет, — и (2) противоречил требованию «Попытка с неотвеченным созданием распознаётся при
повторе» (спека `online-payment`), которое **требует**, чтобы повтор после неизвестного
исхода уходил с тем же идентификатором. Сужение запрета до «новой попытки» снимает
...
Находка P1 «копируемый идентификатор указывает в пустоту» закрыта иначе — решением владельца
от 2026-08-18: показ идентификатора убран целиком, и обсуждать, «когда его показывать»,
больше нечего. Довод, который здесь стоял прежде («показываемый идентификатор всегда тот, что
лежит в `metadata.requestId` у оператора»), сохраняет силу только для **протокола**: клиент
переключается на канонический `requestId` из ответа сервера. Опознание же на стороне
института идёт по `description` в панели ЮKassa — по `metadata.requestId` панель искать не
умеет (наблюдение 2.2a).
...
**CI/preview mock-сборка удержания не создаёт (внесено по Б3; область сужена Решением 13).** Демо-обработчик отвечает
...
отправки CI/preview-формы `/oplata` больше её не показывает. Такой `dist` проверяется
...
отвечает при любых входных данных (Решение 1в). На развёрнутом стенде исключения нет:
тестовый платёж удерживается как боевой.
...
решением владельца от 2026-08-14).** Исход **вытесняет** форму
внутри уже открытого окна, в том числе кнопку отправки, на которой в этот момент стоял
фокус. Объявление нового состояния — обязанность спеки `online-payment` (Requirement
«Диалог оплаты объявлен модальным и управляем с клавиатуры», сценарий «Смена состояния
объявляется без перевода фокуса»): live region / тот же механизм, **без перехвата
фокуса**. Прежняя редакция требовала перенести фокус внутрь панели исхода и
обосновывала это тем, что нормы доступности окна (открытие, ловушка, Escape) не
покрывают смену содержимого. Это было ошибкой: смена содержимого окна — смена состояния
диалога, и эти же нормы её уже покрывают. Исключение «для смены содержимого» не
вводится. Проверяется объявляющей ролью/областью (live region) на панели исхода и тем,
что фокус **не** переведён внутрь неё, — а не `document.activeElement` внутри панели.
...
будет, и состояние обязано оставаться работоспособным без неё — предупреждение и способ
связаться напрямую от неё не зависят.

**Идентификатор попытки посетителю не показывается (решение владельца от 2026-08-18).**
Прежде здесь стояли два абзаца: «копированием, не диктовкой» (UUID v4 нельзя продиктовать по
телефону, поэтому даём копирование одним действием) и разведение «одного действия» с
раскрытием «Технические данные» (кнопка видна, значение под свёрткой). Оба снимаются вместе с
самой возможностью: показывать нечего.

**Причина — наблюдение, а не вкус.** 17–18.08.2026 проверено, что сотрудник **не может** найти
платёж по нашему идентификатору: поиск в панели ЮKassa принимает имя, почту и телефон
покупателя, номер платежа или счёта и номер карты, а `metadata.requestId` не показан ни на
одной из трёх поверхностей карточки платежа (`tasks.md`, 2.2a). Значит продиктованный или
скопированный код не ведёт ни к чему, и весь довод «копированием, а не диктовкой» решал
несуществующую задачу: спор был о **способе** передачи кода, тогда как ложной была сама
передача. Это и есть выбранный вариант развилки «короткий код обращения» (Решение 2е, A4):
сверка признаётся ручным просмотром списка платежей, короткий код не вводится.

**Что осталось у посетителя в состоянии «нужна сверка»:** предупреждение, продолжение попытки
и способ связаться напрямую. Ни одно из трёх от идентификатора не зависело.

**Различение удержаний без UUID.** Перечень попыток нумеруется по порядку («Попытка 1»,
«Попытка 2»), показывает время создания и состояние человеческим языком. Семинар и сумма —
**дополнение при наличии сводки текущей браузерной сессии**, и требовать их безусловно нельзя:
это заставило бы хранить их постоянно, а постоянное хранилище держит только `requestId` и
время создания (Решение 2в). Граница приватности ради удобства перечня не расширяется — прямое
указание владельца. После перезапуска браузера перечень работает по номеру, времени и
состоянию.

**`requestId` остаётся техническим ключом** — удержание, продолжение, проверка статуса,
переключение на канонический идентификатор из ответа сервера. Машинный признак в разметке
(атрибут выбора попытки) показом не является: он не виден и не копируется действием
интерфейса.
...
**Наблюдение выполнено 17–18.08.2026: условие развилки выполнилось. Решение владельца от
18.08.2026 — короткий код НЕ вводится, показ идентификатора убирается.** Разбор ниже
сохранён как обоснование выбора, а не как открытый вопрос.
...
6. **Архивирование разблокировано по этому основанию, но не вообще.** Известно, чему
   ...
   **Зависимость от baseline транспорта заявки, которая здесь стояла (`tasks.md`, 7.8:
   пересекающаяся дельта `request-form-transport` должна быть обновлена относительно
   основной спеки до архивирования), снята решением владельца 2026-08-20/21 иначе, чем
   ожиданием baseline** — возможность `request-form-transport` удалена из этого change
   (не архивируется вместе с ним), а её требования, специфичные для диалога оплаты,
   перенесены в спеку `online-payment` самостоятельными формулировками; пересечения
   областей больше нет. Остаётся обычный Definition of
...
`requestId` (UUID v4) передаётся в заголовок
...
**Резервное копирование — требование прод-контура; на демо-стенде его нет намеренно
(решение владельца от 2026-08-18).** Отдельного диска у демонстрационного VPS нет, служба и
таймер копирования на стенд **не устанавливаются**, потеря тестовой истории при повреждении
VPS принята. Это осознанная граница, а не незакрытая работа, и записана она прямо по двум
причинам: иначе задача 4.3a выглядит невыполненной, а таймер рано или поздно ставят на демо
«для полноты». Из этого же следует, что прод-требование нельзя объявить проверенным на
стенде: там его исполнителя нет вовсе, и приёмка копирования остаётся прод-фазой. Механизм
при этом уже написан и остаётся годным для прод-контура — под одну инстанцию, что для
прод-контура и требуется.

**Для прод-контура: периодическая копия, интервал короче окна ЮKassa.** Файл
...
Развилке 1 `proposal.md` (B2), верное для прод-контура и только для него:** страница,
отдаваемая по HTTPS, браузер блокирует как смешанный контент **любое** обращение со страницы `https://`
...
**Решением владельца от 2026-08-18 смешанный контент на стенде стал невозможен по
построению, а не по совпадению схем.** Стенд объявляет API на своём же origin
(`/api/payments` через обратный прокси), поэтому страница и API совпадают схемой всегда —
включая случай, когда стенд однажды получит HTTPS. HTTPS для API-эндпоинта остаётся
предусловием **прод-контура**, а не предусловием слияния PR #112.
...
данные боевые и деньги реальные.

**Уточнено решением владельца от 2026-08-18: прежний довод «демо-стенд с тестовыми
реквизитами не представляет ценности для такой атаки вовсе» — ложен и снят.** Он был верен,
пока стенд был чистым mock без сети к ЮKassa. Стенд с тестовым магазином `1440249` —
настоящий клиент API оператора платежей, публично доступный по HTTP без аутентификации:
анонимный запрос создаёт настоящую запись в тестовом магазине, ложится в хранилище
идемпотентности на 14 суток, расходует квоту тестового API и засоряет журнал и ручную
сверку. Денег он не списывает — и только это отличает его от прода. Поэтому лимиты на стенде
остаются **fail-closed**, а не «по существу нужны на проде»: ослабление лимита на стенде со
ссылкой на прежний довод было бы регрессией, легализованной формулировкой.
...
защита от автоматических отправок (спека `online-payment`, Requirement «Защита формы
оплаты от автоматических отправок наблюдаема») и наблюдаемость платежей в панели
ЮKassa. Минимальная
...
**Условие выполнилось (наблюдение 17–18.08.2026), и развилка ЗАКРЫТА решением владельца от
18.08.2026: короткий код НЕ вводится.** Обе половины условия проверены и обе отрицательны —
`metadata.requestId` недостижим ни на одной из трёх поверхностей карточки платежа, поиск по
описанию не предусмотрен, и ни одно из полей поиска панели посетитель назвать не может.

**Выбран второй вариант: сверка признаётся ручным просмотром списка платежей, а показ
идентификатора убирается** — вместе с элементом копирования и с UUID в перечне удержаний.
Довод, который решил дело: показ обещал возможность, которой нет, а замещаемый сайт такого
обещания не давал вовсе, поэтому удаление не задевает паритет. Модель хранения при этом не
меняется — короткий код потребовал бы нового хранимого поля с уникальностью, и именно этой
цены удалось не платить.

Статус `Proposed / Not accepted` у самого короткого кода сохраняется — теперь не «пока не
решено», а «решено не вводить». Механика удаления — Решение 2е, требования — спека
(«Состояние „нужна сверка“ удерживает от второй оплаты и даёт, чем себя объяснить» и
требование про перечень удержаний).
...
**Состояний сборки три, а не два — исправлено вторым раундом ревью #112 и уточнено
Решением 13 и решением от 2026-08-19.** Для текста страницы различаются роли: `ci` без формы, `preview` с mock-формой и
установленный контур с рабочей формой (`stand` или `prod`). Без формы описание не обещает
оплату на сайте; при любой форме — называет оплату на сайте. Развёрнутый стенд больше не
является mock-ветвью: он создаёт тестовый платёж. Объявленная роль всё равно проверяется,
потому что определяет endpoint и допустимость публикации, но ожидание FAQ для всех форм
одинаково.
...
### Решение 13: CI может не иметь оплаты; стенд и production — только с работающим магазином

**Решение владельца 2026-08-18, заменяющее прежнее отождествление демо-стенда с
`PAYMENT_MODE=demo`.** Обнаружен не синтаксический, а операционный дефект: production-
артефакт содержал форму и захардкоженный, буквально совпадающий с ожидаемым, адрес,
поэтому прежний build-гейт зелёный, хотя API не развёрнут. Проверять только строку
адреса недостаточно — а сам буквальный адрес, как выяснилось при пересмотре
2026-08-20/21, тем более не может быть неявным умолчанием кода (см. ниже).

Контуры разделяются так:

| Роль | Сборка клиента | Режим сервера | Магазин |
|---|---|---|---|
| обязательный workflow `Tests`, GitHub Pages — роль `ci` | формы нет вовсе, объявленного эндпоинта нет | серверного процесса нет | отсутствует |
| клиентские сценарии и локальный preview — роль `preview` | форма обязательна, ведёт только на mock | mock/`demo`, без сети к ЮKassa | отсутствует |
| установленный `stand` | форма обязательна, объявляемая база — `<origin стенда>/api` | `test`, полный контракт создания и статуса | тестовый `1440249` |
| установленный `prod` | форма обязательна, база — **явно заданный** адрес без встроенного умолчания (адрес не определён этим change, см. ниже) | `prod`, полный контракт создания и статуса | боевой `409285` |

**У роли `prod` нет и не может быть значения адреса по умолчанию (решение владельца
2026-08-20/21).** Production endpoint не выбран этим change (`proposal.md`, Развилка 1,
не принята) — код и деплой-скрипты SHALL требовать `PAYMENT_ENDPOINT_PROD` явно и
завершаться отказом при его отсутствии, а не подставлять исторический
`https://api.ikpk.su` или любой другой захардкоженный адрес. Выбор адреса —
предмет `production-payment-rollout`.

Workflow GitHub Pages `deploy.yml` **входит в первую строку** (решение владельца от
2026-08-18: Pages платёжной поверхностью не является): его артефакт собирается в роли без
активной формы и вместе с формой не несёт обещания оплаты на странице. Прежняя редакция
относила Pages к `prod` и делала работающий прод-эндпоинт предусловием слияния — эта
посылка снята. `test` отличается от `prod` не формой HTTP-контракта и не клиентским UX, а магазином и
тем, что ЮKassa помечает платежи тестовыми. Именно поэтому mock-обработчик нельзя выдавать
за стенд: он не проверяет Basic-авторизацию, создание платежа, checkout и получение
статуса. В то же время секрет тестового магазина не передаётся в GitHub CI — обязательные
проверки используют mock или сборку без формы.

**Одна точка выбора для каждого процесса.** Клиентская сборка получает явную роль
`ci|preview|stand|prod`; устанавливаемый сервер — явную роль `test|prod`; отдельный `demo`
разрешён только процессу роли `preview` и не проходит deploy-гейт. Неизвестное значение —
ошибка. Отсутствие значения допускается только обычной CI-сборкой, означает роль `ci` — то
есть отсутствие формы вовсе — и не является умолчанием ни для `preview`, ни для `stand`, ни
для `prod`.

**Семантика по ролям, и где кончается продукт (решение владельца от 2026-08-19, находка D3).**
Роль определяет поведение клиента, и только его:

| роль | форма | исход отправки | удержание попытки |
|---|---|---|---|
| `ci` | нет | — | — |
| `preview` | mock | демо-исход, терминальный | не создаётся |
| `stand` | рабочая | рабочая семантика платежа | незавершённые исходы удерживаются |
| `prod` | рабочая | та же рабочая семантика | так же |

**Способ исполнения проверок продуктовым требованием не является** и потому переехал в
`tasks.md`: перехват сетевых обращений средствами Playwright — тестовый транспорт, а не режим
продукта. Артефакт роли `stand` с перехваченным API остаётся артефактом роли `stand`: перехват
не меняет ни объявленную роль, ни объявленную базу, ни семантику удержания. Прежняя редакция
предписывала исполнять клиентские сценарии на `preview`, и это делало невыполнимыми около
сорока уже существующих сценариев удержания, сверки, продолжения и подтверждения дубля — на
`preview` удержания нет по норме, на `ci` нет формы.

**Почему роль `preview` выделена из `ci` (решение владельца от 2026-08-19, находка D1 сессии
красных тестов).** Прежние три роли давали `ci` два несовместимых смысла: сборка `Tests` и
Pages без формы и сборка клиентских сценариев с mock-формой. Дельта к `deploy-gating`
требует выводить ожидаемое число форм и адрес **из роли** — а для такой `ci` объективного
критерия нет, любой ответ проверки обоснован. Альтернатива «`ci` плюс флаг mock=да/нет»
отвергнута владельцем: она возвращает ту же неоднозначность, только в другое место, и снова
делает роль недостаточной.

**Две установленные инстанции, а не переключение одного процесса.** Стенд и production
используют один код, но разные systemd-инстанции (либо эквивалентно изолированные
процессы), env-файлы и каталоги состояния. **Про порты — уточнение владельца от 2026-08-18:**
стенд слушает `127.0.0.1:8787`, и это задаётся явно, а не умолчанием; конфликта с
прод-контуром здесь нет, потому что production на демонстрационном VPS не разворачивается, а
один и тот же loopback-порт на **разных** host допустим. Требование разных портов вступает в
силу, только если stand и prod однажды окажутся на одном host, и тогда порты назначаются
отдельным решением. Правило `root:root 0600` Решения 1
применяется к каждому env-файлу отдельно; изменяемые записи и canary каждого режима
лежат в своём каталоге. Переключить работающий production-процесс на тестовый ключ для
приёмки или разделить одно хранилище между режимами запрещено: одинаковый `requestId`
в двух контурах должен означать две независимые записи.

**Личность контура проверяется, а не предполагается по адресу.** Серверный health/readiness-
ответ публикует только несекретные признаки: режим и `shopId`. Перед публикацией клиента
деплой проверяет одновременно:

1. в артефакте есть ровно одна активная платёжная форма — по признакам, названным в спеке
   (опознавательный атрибут, объявленная база, её буквальное равенство ожидаемой для роли,
   объявленная роль `stand` либо `prod`);
2. объявленный эндпоинт доступен — проба `OPTIONS <объявленная база>/payments` с ожиданием
   `204` (решение владельца от 2026-08-19, находка A2). Проба выбрана именно такой, потому
   что она ничего не создаёт: `POST /payments` создал бы платёж, `GET .../status` требует
   существующей попытки, а `/readyz` наружу не публикуется и достижимость внешнего маршрута
   не доказывает. Текущий backend `OPTIONS` уже обслуживает. **Схема не предписывается:** для
   нынешнего стенда это HTTP (`http://193.124.115.99`), требование HTTPS относится к
   прод-контуру (решение владельца от 2026-08-18);
3. readiness сообщает `test/1440249` для `stand` либо `prod/409285` для production; запрос
   идёт **изнутри host инстанции** (`http://127.0.0.1:8787/readyz`), а не через публичный
   эндпоинт — наружу readiness не публикуется;
4. для production тот же `OPTIONS` служит и CORS-проверкой с origin боевого сайта. Для стенда
   её нет: запрос same-origin, и браузер CORS к нему не применяет.

Неуспех любого пункта останавливает выкладку. Нет ветви «временно убрать форму» для
`stand` или `prod`, нет автоматического перехода на другой магазин и нет fail-open по
непрочитанному readiness-ответу. Уже опубликованный контур при последующей аварии API
показывает предусмотренное клиентом состояние ошибки; он не меняет конфигурацию и не
переключается на mock.

**CI-артефакт нельзя случайно выпустить.** Отсутствие формы в CI является разрешённым
исходом сборки, но deploy-гейт `stand|prod` трактует ноль платёжных форм как отказ — как
и прежде трактовал «проверять нечего». Тем самым CI не требует секретов и живого API,
но его `dist` не может стать релизом установленного контура.

Альтернатива «форма есть всегда, доступность проверим после публикации» отвергнута:
она уже привела к ссылке на несуществующий `api.ikpk.su`. Альтернатива «выключатель в
production/stand» отвергнута владельцем: эти два режима определяются именно как рабочие
платёжные контуры; неготовность зависимости блокирует их деплой.

## Risks / Trade-offs

- **Стенд случайно использует боевой магазин либо production — тестовый** → Решение 13:
  разные env-файлы, явная роль процесса, проверка точного `shopId` при старте и повторная
  readiness-проверка deploy-гейтом. Проверки только URL недостаточно: один и тот же API
  ЮKassa обслуживает оба магазина.
...
- **`payment-request-flow` замещён до реализации, а его требование
  ...
  решено сначала (P1-4а) копированием текста в `online-payment-flow` как собственную
  возможность; **решено иначе 2026-08-20/21**: копия описывала уже существующий сайт, не
  работу этого change, и удалена отсюда — специфичное для диалога оплаты сформулировано
  самостоятельно в спеке `online-payment`, а настоящее место остального —
  будущий `baseline-request-form-transport`.
...
- **Оракул отпечатка — named deviation, принятый владельцем 2026-08-13 целиком и названный
  ...
  оплатить тот же состав; **(4)** сам `requestId` не доказывает ни владения попыткой,
  ни факта оплаты (посетителю он не показывается, но остаётся в ответе сервера); **(5)** сотрудник института не должен считать один названный `requestId`
  ...
  автоматических отправок (спека `online-payment`), отпечаток как HMAC с серверным ключом
...
1. Владелец решил три из четырёх развилок `proposal.md` (формулировка согласия —
   временная сейчас, подтверждение до прод; 54-ФЗ — до тест-сессии; change не делится) и
   утвердил рантайм (Решение 1) и TTL (Решение 4, 14 суток). **Четвёртая (прод-origin,
   Развилка 1) пересмотрена 2026-08-20/21:** записанное 2026-08-11 решение
   (`api.ikpk.su`) не принимается этим change — выбор production endpoint отложен до
   `production-payment-rollout`, объём этого change заканчивается демо-стендом и
   production-capable кодом без выбранного production-адреса.
...
5. Выкатка: демо-стенд с тестовым магазином `1440249` — это полный тестовый путь, а не
   mock-режим, и это выкатка, которой заканчивается объём этого change. Боевая выкатка
   (адрес и топология production API, HTTPS, CORS, сервис с боевым магазином `409285`)
   — объём `production-payment-rollout`, отдельного будущего change. Для установленного
   контура до публикации проходит readiness-проверка режима и `shopId` (Решение 13);
   отсутствие формы либо API блокирует деплой, а не отключает оплату — это правило
   действует и для будущей боевой выкатки, не только для стенда.
