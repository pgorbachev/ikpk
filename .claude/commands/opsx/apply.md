...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If `state: "blocked"` (missing artifacts): show message, suggest using `/opsx:continue` (if it is not installed, run `./bin/openspec status --change "<name>" --json` to see the next artifact and `./bin/openspec instructions <artifact-id> --change "<name>" --json` for how to create it)
