...
/**
 * Режим ИМЕННО заглушки, а не «любой демо-режим».
 *
 * Отличать пришлось после ложного отказа выкладки: страница `/demo-zayavka`
 * собиралась при любом непустом `DEMO_FORMS`, в том числе когда формы уведены на
 * свой тестовый портал и на заглушку не ведёт ничто. Лишняя страница приносила
 * `<link rel="canonical">` на `https://ikpk.su/demo-zayavka` — адрес, которого на
 * боевом сайте нет, — и гейт форм справедливо считал его ссылкой, не отвечающей
 * объявленному режиму.
 */
export const isDemoStub = mode === 'stub';
