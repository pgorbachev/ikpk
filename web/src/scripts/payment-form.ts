...
function boot(formEl: HTMLFormElement) {
  ...
  function restoreFieldsDom() {
    ...
    formEl.hidden = false;
    // Форма вернулась в разметку — вернулся и футер, причём без показанных ошибок, то есть
    // другой высоты. Всякий путь, меняющий геометрию, обязан пересчитать полосу: иначе
    // остаётся решение, принятое по прошлым размерам (то же, что нашло ревью в `validate`).
    syncFooterBand();
  }
