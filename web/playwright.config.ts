...
export default defineConfig({
  ...
  timeout: 10000,
  // JSON-отчёт включается ТОЛЬКО когда его просят переменной, и она же задаёт путь.
  // Так сделано ради одного гейта: `scripts/check-month-run.ts` сверяет объявленные метки
  // месяца с ПРОГОНОМ, а не со сбором, и предмет его проверки — отчёт того самого запуска.
  // Через аргумент командной строки этого делать нельзя: мета-гейт собирает набор тем же
  // вызовом с добавленными `--list --reporter=json`, и второй `--reporter` в строке сделал
  // бы предмет сверки зависящим от порядка флагов.
  ...
  projects: [
    ...
    // Третья полоса ширины подвала: 481–768 px включительно. Проектов было два — 1280 и
    // 375, — и в эту полосу не попадал ни один, то есть требование о двух колонках не
    // проверялось ничем. Проект узкий по составу намеренно (`testMatch` на один файл):
    // прогонять весь набор на третьей ширине эта работа не заказывала, а расширение
    // состава двигало бы инварианты числа браузерных тестов.
    {
      name: 'footer-tablet',
      use: { viewport: { width: 600, height: 800 } },
      testMatch: '**/footer-social.spec.ts',
    },
    {
