...
const REQUIRED_GZIP_GROUPS = [
  ...
  // Замечание для реализации: mime.types в nginx Ubuntu отображает `.xml` в `text/xml`.
  // Перечень с одним `application/xml` карту сайта сжимать НЕ будет, а этот тест
  // пройдёт — фактический тип определяется таблицей типов, а не нашим конфигом.
  // Отсюда сжатие `/sitemap-index.xml` проверяется на живой раздаче (раздел 5).
...
describe('раздача: кеш-политика и сжатие в тексте генерируемого vhost', () => {
  ...
  // 2.4 — удвоение. Мутация 4.5.
  it('2.4 ни один блок не объявляет Cache-Control дважды', () => {
    const parsed = cfg();
  ...
  // запрещает это изменить. Мутации в разделе 4 `tasks.md` на неё нет — пробел назван в
  // отчёте сессии тестов; подходящая мутация — добавить в `location /` свой
  // `add_header Cache-Control`, тогда правила перенаправления потеряют политику.
  ...
  it('2.6 gzip_types задан и покрывает CSS, JavaScript, SVG, XML, JSON, обычный текст', () => {
    ...
    const missing = REQUIRED_GZIP_GROUPS.filter(
      (g) => !g.accept.some((t) => types.includes(t)),
    ).map((g) => `${g.what} (годится любой из: ${g.accept.join(', ')})`);
    expect(missing, `в gzip_types нет типов:\n${missing.join('\n')}\nЗадано: ${types.join(' ')}`).toEqual(
      [],
    );
  });
  ...
  // сейчас — проверка характеризационная. Мутации в разделе 4 `tasks.md` на неё нет:
  // пробел назван в отчёте сессии тестов, подходящая мутация — добавить add_header в
  // шаблон генератора `web/scripts/gen-redirects.ts`.
  ...
  // Сценарий «Правил перенаправления пока нет»: механизм — файл создаётся ДО nginx -t,
  // потому что include одного отсутствующего файла есть ошибка конфигурации, а пустой
  // include законен. Проверки на этот сценарий tasks.md не назначает — добавлена здесь.
 ...
 *   `Accept-Encoding` (сценарий «Клиент без поддержки сжатия») в разделе 5 шага не имеет:
 *   все команды 5.1–5.7 идут с `-H 'Accept-Encoding: gzip'`. Это пробел раздела 5,
 *   названный в отчёте сессии тестов, а не покрытая проверка.
 *
 * Требование «Введение политики не ухудшает уже достигнутое кеширование»
 *   «Шрифт берётся из кеша без обращения к сети» требует браузера с прогретым кешем;
 *   в разделе 5 такого шага нет, есть только его текстовая причина — `max-age=2592000`
 *   у класса `/fonts/` (проверено 2.2) и наблюдаемый заголовок (5.3). Сценарий
 *   «Принятая плата названа» документарный: он удовлетворён `design.md` (Решение 7) и
 *   проверяется чтением, а не прогоном.
 *
