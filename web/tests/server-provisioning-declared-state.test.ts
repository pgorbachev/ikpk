...
/**
 * РЕГРЕСС. `scripts/bootstrap-vps.sh` объявляет инвариант своими словами: «Код остаётся у
 * root: служба не должна иметь права переписать то, что исполняет», и перечисляет
 * писчие каталоги поимённо. Инвариант держался на комментарии и не стерёгся ничем.
 *
 * Сломать его оказалось легко и незаметно: каталог `current/database` отдавали службе,
 * пока он был ПУСТ (Strapi создаёт его при старте, иначе EACCES), и это ничего не стоило.
 * Как только в него поехали миграции — исполняемый код, подключаемый через `require` при
 * каждом запуске, — служба получила право переписывать то, что исполняет. Все 74 теста
 * над этими же скриптами при этом остались зелёными.
 *
 * Предмет проверки — не точная строка, а СВОЙСТВО: под корнем релиза службе разрешён
 * только служебный кэш. Данные и загрузки лежат вне релиза и проверяются отдельно.
 */
describe('служба не может переписывать то, что исполняет', () => {
  const unit = readFileSync(join(REPO_ROOT, 'scripts', 'bootstrap-vps.sh'), 'utf-8');

  function readWritePaths(): string[] {
    const line = unit.split('\n').find((l) => l.startsWith('ReadWritePaths='));
    if (!line) throw new Error('в юните нет ReadWritePaths — проверять нечего');
    return line.slice('ReadWritePaths='.length).trim().split(/\s+/).filter(Boolean);
  }

  it('под корнем релиза службе разрешён только .strapi', () => {
    const underRelease = readWritePaths().filter((p) => p.includes('/current/'));
    expect(
      underRelease.map((p) => p.slice(p.indexOf('/current/') + '/current/'.length)),
      'службе отдан каталог внутри релиза: там лежит исполняемый код, и она сможет его переписать',
    ).toEqual(['.strapi']);
  });

  it('каталог миграций не передаётся учётной записи службы', () => {
    // Команда переносится через `\`, поэтому продолжения надо СКЛЕИТЬ. Первая редакция
    // этой проверки брала только строки, начинающиеся с `chown`, — и мутация, вернувшая
    // `${new_release}/database` на следующую строку, оставила её зелёной. Проверено
    // повторно: теперь краснеет.
    const joined = unit.replace(/\\\n\s*/g, ' ');
    const chowns = joined.split('\n').filter((l) => l.includes('chown -R "${SERVICE_ACCOUNT}'));

    expect(chowns, 'нет ни одной строки chown на учётную запись службы — проверка вакуумна').not.toEqual(
      [],
    );
    const offenders = chowns.filter((l) => l.includes('${new_release}/database'));
    expect(
      offenders,
      'каталог database отдан службе: в нём лежат миграции, то есть исполняемый код, ' +
        'который Strapi подключает при каждом запуске',
    ).toEqual([]);
  });
});

describe('долгое ожидание не превращает канал в тишину', () => {
  const script = readFileSync(join(REPO_ROOT, 'scripts', 'bootstrap-vps.sh'), 'utf-8');

  // Объявленный на стенде срок ожидания службы — 1800 с (`deploy/environments/stand.env`).
  // Цикл ожидания сам по себе не пишет ничего: `curl -s -o /dev/null` и `sleep`. Полчаса
  // молчания в ssh-сессии — это две беды сразу: промежуточный NAT или firewall с
  // idle-таймаутом вправе порвать такую сессию (класс TD-59 — выкладка падает от одного
  // оборванного ssh), а оператор не отличает ожидание от зависшего прогона.
  it('цикл ожидания службы подаёт признак жизни', () => {
    // Срез — по ТЕЛУ цикла, а не по окрестности: строка, поставленная перед `while`, печатается
    // один раз и тишины не снимает, а прежний срез (от `health_deadline=` до `if ((healthy))`)
    // такую подмену принимал.
    const from = script.indexOf('while ((SECONDS < health_deadline)); do');
    const to = script.indexOf('    done', from);
    expect(from, 'цикл ожидания не найден — проверять нечего').toBeGreaterThan(-1);
    expect(to, 'конец цикла ожидания не найден — проверять нечего').toBeGreaterThan(from);
    const loop = script.slice(from, to);
    expect(
      loop.includes('жду ответа службы'),
      'цикл ожидания снова молчит весь срок: полчаса без байта рвут ssh-сессию и слепят оператора',
    ).toBe(true);
  });
});
