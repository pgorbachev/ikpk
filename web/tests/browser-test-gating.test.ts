 ...
 *
 * Граница списка названа вслух: это ПЕРЕЧЕНЬ ЧАСТНЫХ СЛУЧАЕВ, а такой приём отстаёт от
 * предмета молча. Проверка месяца, написанная через `getByLabel('Месяц')` или
 * `data-testid="schedule-filter-month"`, условием про место не поймается — то есть
 * сценарий, уехавший в файл из списка долга в таком начертании, здесь пройдёт незамеченным.
 * Расширять список именно поэтому бессмысленно: следующее начертание опять не войдёт.
 * Держать это допустимо по одной причине: потерю ОБЪЯВЛЕННОГО сценария ловит не эта
 * проверка, а симметричная сверка меток с собранным набором и сверка с прогоном — они не
 * зависят ни от одного текстового признака. То есть цена промаха здесь — «сценарий
 * исполняется, но не оттуда, откуда ожидалось», а не «сценарий пропал».
 */
...
/**
 * Явный таймаут проверок, которые собирают набор. Дефолтные 5000 мс vitest здесь не
 * величина по существу, а ложное красное: длительность такой проверки — это длительность
 * подпроцессов `playwright --list`, и в обязательном прогоне они соревнуются за машину с
 * пятнадцатью другими файлами. Ровно этот класс уже записан в `docs/tech-debt.md` как
 * TD-18, и лечение то же самое. Запас берётся от бюджета самого подпроцесса
 * (`COLLECT_SPAWN_TIMEOUT_MS` ниже), а не от наблюдённого времени: при зависшем
 * `playwright` первым обязан сработать таймаут подпроцесса — он говорит, ЧТО зависло,
 * а таймаут теста сказал бы только «проверка не уложилась».
 *
 * Большой таймаут тут ничего не ослабляет: стерегомый дефект (метка объявлена, а
 * выполняемого теста под неё нет) определяется СОДЕРЖИМЫМ собранного набора, а не
 * временем; на успехе проверка возвращается сразу.
 */
const COLLECT_SPAWN_TIMEOUT_MS = 120_000;
const COLLECT_TEST_TIMEOUT_MS = COLLECT_SPAWN_TIMEOUT_MS * 2 + 30_000;
...
function collectTests(all: Workflow[]): CollectedTest[] {
  ...
  for (const args of gatingPlaywrightArgs(all)) {
    ...
    const run = spawnSync(PLAYWRIGHT_BIN, ['test', ...args, '--list', '--reporter=json'], {
      cwd: WEB_DIR,
      encoding: 'utf-8',
      timeout: COLLECT_SPAWN_TIMEOUT_MS,
...
describe('браузерные проверки и гейт публикации', () => {
  ...
  it('под каждую объявленную метку в собранном наборе есть выполняемый тест', { timeout: COLLECT_TEST_TIMEOUT_MS }, () => {
  ...
  it('в собранном наборе нет меток месяца, которых нет в объявлении', { timeout: COLLECT_TEST_TIMEOUT_MS }, () => {
  ...
  it('у каждого сценария месяца есть метка — непомеченный гейту невидим', { timeout: COLLECT_TEST_TIMEOUT_MS }, () => {
