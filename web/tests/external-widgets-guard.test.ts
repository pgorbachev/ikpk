...
import { SEL_AWARD_ROW, SEL_CHAT_MOUNT, SEL_CHAT_TRIGGER, SEL_REVIEWS_SECTION } from './helpers/external-widgets';
...
/** Отдельный прогон повторяемости снимков — он обязан стоять в гейте, а не в долге. */
const REPEAT_RUN = 'external-widgets-baseline-repeat.spec.ts';
/** Прогон эталонов: он в списке признанного долга, и повторяемость в него класть нельзя. */
const BASELINE_RUN = 'visual-baseline.spec.ts';
...
/** Код файла без комментариев: проверка не должна краснеть от собственного пояснения. */
...
describe('сторонние встраивания не попадают живыми ни в один браузерный прогон', () => {
  ...
  it('подставных ответов виджета ДВА, и оба объявлены в одном месте со списком хостов', () => {
    // Без этого у проверки различения счётчиков нет построимого красного состояния:
    // живого ответа в прогонах нет по требованию о перехвате, значит пиксель обязан
    // прийти из подставного ответа, а какой именно ответ его несёт — должно быть сказано.
    //
    // Признак — ИМЕНОВАННЫЕ объявления в модуле перехвата, а не «где-то в тестах»: спека
    // требует, чтобы оба были объявлены в одном месте вместе со списком хостов.
    const file = join(WEB, GUARD_MODULE);
    expect(existsSync(file), `нет '${GUARD_MODULE}'`).toBe(true);
    const code = codeOf(file);
    for (const name of ['FIXED_SIZE_WIDGET_BODY', 'WIDGET_BODY_WITH_FOREIGN_PIXEL'])
      expect(
        new RegExp(`(const|function)\\s+${name}\\b`).test(code),
        `в модуле перехвата нет объявления '${name}': подставных ответов виджета два — ` +
          'фиксированного размера для проверки сдвига раскладки и несущий пиксель чужого ' +
          'счётчика для проверки различения, — и оба обязаны быть названы',
      ).toBe(true);
    expect(
      /THIRD_PARTY_HOSTS|hosts/i.test(code),
      'в модуле перехвата нет перечня хостов: спека требует объявить подставные ответы ' +
        'ВМЕСТЕ со списком хостов, в одном месте',
    ).toBe(true);
  });
...
describe('повторяемость снимков живёт отдельным прогоном, а не внутри прогона эталонов', () => {
  it('отдельный прогон повторяемости существует', () => {
    ...
    // workflow, а не внутри `visual-baseline.spec.ts` — тот числится в списке признанного
    // долга, и принятое правило запрещает файлу быть одновременно в гейте и в долге.
    expect(
      existsSync(join(TESTS_DIR, REPEAT_RUN)),
      `нет '${REPEAT_RUN}': без него у сценария повторяемости не было бы автоматической ` +
        'красноты вовсе — он читался бы как принадлежащий файлу в долге',
    ).toBe(true);
  });

  it('прогон эталонов повторяемости в себя не забирает', () => {
    // Обратная сторона: положить проверку в файл из долга — то же, что не написать её.
    const file = join(TESTS_DIR, BASELINE_RUN);
    expect(existsSync(file), `нет '${BASELINE_RUN}'`).toBe(true);
    const code = codeOf(file);
    expect(
      /дважды|повторяем|один и тот же подставленный момент/i.test(code),
      `в '${BASELINE_RUN}' появилась проверка повторяемости: этот файл в списке признанного ` +
        'долга, значит ни один гейтующий workflow его не исполняет, и проверка была бы ' +
...
describe('известные отклонения записаны, а не оставлены молчанием', () => {
  ...
  it('недетерминированность визуальных эталонов вне CI названа отклонением', () => {
    ...
    // сценарий «недоступность сторонних сервисов не меняет результат» не стережёт ничто в
    // CI. Умолчать нельзя: следующий читатель примет требование за охраняемое.
    //
    ...
    const hit = sections().filter((s) => namesThisCapability(s) && /visual-baseline\.spec\.ts/.test(s));
    expect(
      hit.length,
      'нет записи о том, что сценарий недоступности сторонних сервисов не охраняется в CI: ' +
  ...
  it('недоступность часов работы при неответившем загрузчике названа отклонением', () => {
    // Сужение, названное спекой прямо: прежняя редакция обещала часы и телефон, НЕ требуя
    // открывать чат. Теперь часы доступны только после нажатия и успешного ответа
    // стороннего загрузчика — значит вне страниц контактов и оплаты часы узнать нельзя, и
    // это записано известным отклонением, а не выполненным требованием.
    const hit = sections().filter(
      (s) =>
        namesThisCapability(s) &&
        /(?<![а-яёa-z])час/iu.test(s) &&
        /(загрузчик|не ответил|недоступн)/i.test(s),
    );
    expect(
      hit.length,
      'нет записи о том, что при неответившем загрузчике часы работы на обычной странице ' +
        'узнать нельзя. Формулировка «наша гарантия — телефоны в подвале» тихо роняет ' +
        'половину прежнего обещания, и умолчание об этом читается как выполненное требование',
    ).toBeGreaterThan(0);
  });
});

describe('датозависимый фрагмент назван поимённо и исключён из сравнения облика', () => {
  const visual = (): string => {
    const file = join(TESTS_DIR, BASELINE_RUN);
    expect(existsSync(file), `нет ${BASELINE_RUN}`).toBe(true);
    return readFileSync(file, 'utf-8');
  ...
  it('датозависимый фрагмент назван в самой проверке, и он ОДИН', () => {
    // Фрагмент один — строка знаков наград. Предыдущая редакция этой проверки требовала
    // назвать ДВА и вторым считала блок часов работы: он снят спекой, и требование «имя
    // снятого блока стоит в прогоне эталонов» стало неисполнимым.
    const code = visual();
    expect(
      code.includes(SEL_AWARD_ROW),
      `в проверке облика не назван датозависимый фрагмент '${SEL_AWARD_ROW}': исключение по ` +
        'признаку («всё, что зависит от даты») не имеет исполнимой формы',
    ).toBe(true);
    expect(
      code.includes('data-chat-hours'),
      'в проверке облика назван снятый блок часов: своего блока часов у нас нет, спека его ' +
        'запрещает, и исключение несуществующего фрагмента — декорация',
    ).toBe(false);
  });
