 ...
 *
 * ПОЧЕМУ ПРОВЕРКИ БЫЛИ КРАСНЫМИ ДО РЕАЛИЗАЦИИ — по каждой проверке отдельно, в её
 * собственном комментарии (ниже — свидетельство, не текущее состояние).
 */
...
describe('состав внешних аккаунтов: единый источник', () => {
  /**
   * ПОЧЕМУ БЫЛА КРАСНОЙ ДО РЕАЛИЗАЦИИ: в перечне было шесть записей, а требование — четыре
   * (`web/src/lib/social.ts`, `export const SOCIAL_LINKS`). После раздела 2 — четыре.
   *
  ...
  /**
   * ПОЧЕМУ БЫЛА КРАСНОЙ ДО РЕАЛИЗАЦИИ: адрес имел вторые определения —
   * литералы `VK_COMMUNITY_URL` / `RUTUBE_CHANNEL_URL` вне `hrefOf`. После 2.3 — единый
   * источник через `hrefOf`.
   *
...
describe('состав внешних аккаунтов: покрыта каждая роль артефакта обязательного прогона', () => {
   ...
   * проверки состава по выводу для всех ролей ещё не было (в т.ч. стенд).
   *
...
describe('отличимость Youtube и Rutube: порог цветового расстояния объявлен и различает цвета', () => {
   ...
   *
   * ЧТО ЗДЕСЬ НЕСУЩЕЕ, СКАЗАНО ПРЯМО, потому что это видно только измерением. Несут два
   * утверждения — значение якоря и значение множителя; сверка `b == round(a × множитель)`
   * **тавтологична**: `CONTROL_PAIR.b` этим же выражением и вычисляется, поэтому покраснеть
   * она может только от правки самой формулы. Оставлена намеренно — она стережёт формулу от
   * подмены на литерал, — но выдавать её за проверку правила нельзя.
   *
   * Обе несущие проверены мутацией, и вторая из них показала, зачем якорь фиксирован числом:
   *
   * | Мутация | Итог |
   * |---|---|
   * | `CONTROL_PAIR_FACTOR` 0,8 → 0,5 | падает на множителе |
   * | `CONTROL_ANCHOR` `#ff0000` → `#111111` | падает на якоре, а проверка расстояния **проходит** |
   *
   * Вторая строка и есть измеренное доказательство лазейки, которую закрывает требование: у
   * тёмного якоря `#111111` пара `#111111`/`#0e0e0e` даёт расстояние около 1,1, то есть порог
   * «подтверждается» по построению, и одна проверка расстояния этого не видит. Правило
   * «фирменный цвет одной из сетей ×0,8» было дефектом промежуточной редакции спеки именно
   * поэтому.
   *
...
describe('реестр применимости марок: запись читается автоматикой, а не человеком', () => {
  ...
  it('как только реестр появится, браузерная проверка обязана его читать', () => {
    ...
    // Признак — ИМПОРТ модуля или ВЫЗОВ предиката, а не упоминание имени в тексте. Разница
    // не теоретическая: первая редакция этого мета-гейта искала подстроку
    // `social-marks-registry`, и негативная мутация (создать модуль) оставила её ЗЕЛЁНОЙ —
    // потому что тот же путь стоит в сообщении assert'а самой браузерной проверки, а
    // `stripComments` убирает комментарии, но не строковые литералы. Гейт закрывался тем же
    // текстом, изменения которого требовал.
    const readsRegistry =
      /(?:from|import)\s*\(?\s*['"][^'"]*social-marks-registry/.test(code) ||
      /\bnetworksRequiringMark\s*\(/.test(code);
    expect(
      readsRegistry,
      'реестр применимости появился, а браузерная проверка подачи его не читает — она ' +
        'по-прежнему требует марку от всех четырёх сетей и объявит нарушением законный выбор ' +
        'владельца. Перечень надо брать из networksRequiringMark(...) (Requirement «Решение о ' +
