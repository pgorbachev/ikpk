...
// ═════════════════════════ Общий срок съёма, отдельно от срока одного запроса ════════════

describe('съём отказывается сам, когда общий срок вышел', () => {
  // Предмет отдельный от предела на ОДИН запрос. Подняв тот предел до 120 с (медленная
  // машина стенда: teachers 18,3 с, articles 16,2 с даже прогретыми), мы сделали достижимым
  // состояние, в котором съём идёт часами: живой снимок тянет 132 медиафайла плюс
  // постраничные выборки по десяти типам. Пусковой скрипт убил бы такой прогон по своему
  // часовому сроку (`scripts/publication-launcher.mjs`, `timeout: 3_600_000`), а убитый по
  // сроку процесс даёт `refuse('publication-worker-failed')` — ТОТ ЖЕ код отказа, что у
  // упавшего работника. Оператор не отличил бы «съём не уложился» от «работник сломался»,
  // то есть «не смогла проверить» выдавалось бы за вердикт.
  it('отказ называет предмет и числа, а не молчит до убийства по чужому сроку', async () => {
    const cms = await stub();
    const dir = outDir();

    const run = await runCapture({
      CMS_URL: cms.url,
      CONTENT_SNAPSHOT_DIR: dir,
      CAPTURE_TOTAL_BUDGET_MS: '0',
    });

    expect(run.status, `съём прошёл, хотя срок был исчерпан:\n${run.output}`).not.toBe(0);
    expect(run.output, 'отказ не назвал предмета').toMatch(/живой захват не уложился в общий срок/);
    // Числа в отказе — не украшение: без них оператор не отличит «срок мал» от «CMS стоит».
    expect(run.output, 'в отказе нет предела').toMatch(/при пределе \d+ с/);
    expect(existsSync(join(dir, 'snapshot.json')), 'снимок записан при отказе').toBe(false);
  });

  // Контроль: без него первый сценарий доказывал бы лишь то, что заглушка не отвечает.
  it('та же заглушка при обычном сроке проходит — значит отказ даёт срок, а не оснастка', async () => {
    const cms = await stub();
    const dir = outDir();

    const run = await runCapture({ CMS_URL: cms.url, CONTENT_SNAPSHOT_DIR: dir });

    expect(run.status, `съём не прошёл на исправной заглушке:\n${run.output}`).toBe(0);
    expect(existsSync(join(dir, 'snapshot.json')), 'снимок не записан').toBe(true);
  });
});
