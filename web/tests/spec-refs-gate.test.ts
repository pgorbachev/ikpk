...
describe('гейт ссылок: исходы по классам', () => {
  ...
  it('при файле слева негоден ЛЮБОЙ хвост, а не перечень символов', () => {
    // Перечень допустимых символов был очередным списком частных случаев: `@not!sha`, `@not+sha` и
    // `@not:sha` уходили в тишину — измерено на реальном репозитории, вывод гейта с тремя такими
    // пробами совпал с базовым побайтово. Слева доказанный файл индекса, значит после `@` стоит
    // ревизия и ничто иное.
    for (const tail of ['not!sha', 'not+sha', 'not:sha', 'не-sha', 'v1.2.3', '~HEAD^2']) {
      const r = run(
        sandbox,
        `## P\n\nСсылка: \`web/src/thing.ts@${tail}\`, и проверяемая \`web/src/thing.ts:1\`, \`marker\`.\n`,
      );
      expect(r.code, `${tail}: ${r.out}`).toBe(1);
      expect(r.out, tail).toMatch(/не полным 40-символьным SHA/);
      // Сообщение обязано называть ХВОСТ ЦЕЛИКОМ. Без этого утверждения ветвь «номер строки
      // отделяется по цифровому виду» ничем не охранялась: при отделении по первому двоеточию
      // вердикт тот же (расхождение), но `not:sha` в отчёте превращается в `not`, и автор ищет в
      // тексте строку, которой там нет. Поймано зелёной мутацией круга 10.
      expect(r.out, tail).toContain(`(${tail}, ${[...tail].length} симв.)`);
    }
  });

  it('известное имя ref слева — охрана по символам сохранена, и это названная асимметрия', () => {
    // `main`, `feature`, `next` — обычные слова, они встречаются в прозе, и пример негодной формы
    // `main@…` стоит в самих правилах. Поэтому для ref-ов охрана по символам остаётся, а для файлов
    // снята. Асимметрия закреплена тестом, иначе её «упростят» обратно в одну ветвь.
    const asProse = run(
      sandbox,
      '## P\n\nПример негодной формы: `main@…`. Ссылка: `web/src/thing.ts:1`, `marker`.\n',
    );
    expect(asProse.code, asProse.out).toBe(0);
    const asFile = run(
      sandbox,
      '## P\n\nСсылка: `web/src/thing.ts@…`, и проверяемая `web/src/thing.ts:1`, `marker`.\n',
    );
    expect(asFile.code, asFile.out).toBe(1);
  });

  it('двоеточие в хвосте не выводит ссылку из разбора, номер строки остаётся цифровым', () => {
    // Двоеточие было исключено из класса хвоста самим регулярным выражением, поэтому `@not:sha`
    // не разбирался вовсе — предмет пропадал не по решению, а по форме разбора. Отделять номер
    // строки должно то, что он ЦИФРОВОЙ.
    const sha = execFileSync('git', ['-C', sandbox, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim();
    const withLines = run(
      sandbox,
      `## P\n\nСсылка: \`web/src/thing.ts@${sha}:1\`, \`marker\`.\n`,
    );
    expect(withLines.code, withLines.out).toBe(0);
    expect(withLines.out).toMatch(/содержимое сверено/);
  });
});
