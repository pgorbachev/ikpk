...
/**
 * Скрипт шага БЕЗ shell-комментариев.
 *
 * Комментарий в теле `run:` — не исполняемая команда, и гейты, разбирающие текст шага,
 * обязаны это различать. Иначе строка вида `# npm run build:demo` внутри пояснения делает шаг
 * «шагом демо-сборки», а пояснение про `astro build` в чужом workflow — «выносом демо-сборки
 * за гейт». Оба случая — ложное красное на исправной конфигурации, и оба возникают от
 * обычного комментирования, то есть от работы, которая ничего не ломает.
 *
 * Разбор посимвольный, с учётом кавычек: `#` внутри `'…'` или `"…"` комментарий не начинает
 * (`jq '.a["#b"]'`), и `#` в середине слова тоже (`sha#1` — не комментарий).
 *
 * ГРАНИЦА НАЗВАНА: строка разбирается независимо от соседних, поэтому кавычка, открытая на
 * одной строке и закрытая на другой (heredoc, многострочный литерал), учитывается как
 * закрытая в конце строки. `#` внутри такого блока будет снят как комментарий. Для наших
 * workflow это безвредно (внутри heredoc лежат скрипты, где `#` и есть комментарий), но
 * держать это молча нельзя: полный разбор shell тут был бы отдельной работой.
 */
