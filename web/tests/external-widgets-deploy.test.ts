...
describe('выкладка стенда: своё ожидаемое и то же выделенное значение', () => {
  it('адрес задан, а вывода нет — выкладка стенда НЕ проходит', async () => {
    // Перевёрнуто решением владельца 2026-09-05: стенд стал прод-лайком, и «адрес
    // объявлен, а встраивания нет» означает то же, что на бою, — сборка не несёт
    // заказанного. Прежняя редакция считала это нормой стенда, потому что стенд гасил
    // встраивания; теперь гасится только невыкладываемый `dist-demo`.
    const dir = distRaw({ 'index.html': '<a href="/demo-zayavka">Записаться</a>' });
    const run = await chatWidget(dir, 'stand', LOADER_OFF_PORTAL);
    expect(run.code, 'стенд принял сборку без заказанного встраивания').toBe(1);
    expect(run.stderr).toMatch(/ожидает встраивание чата/i);
  });
  ...
  it('встраивание на стенде при объявленном адресе выкладку ПРОХОДИТ', async () => {
    // Обратная сторона того же переворота. Стенд прод-лайк: приёмщик обязан увидеть то,
    // что увидит посетитель. Защита от «живого чата на стенде» переехала из ветви
    // сборки в АДРЕСАТА — загрузчик стенда указывает на тестовый портал, а не на портал
    // заказчика, и это вход оператора, который гейт проверить не может.
    const dir = distRaw({
      'index.html': `<a href="/demo-zayavka">Записаться</a>\n${withChat(LOADER_OFF_PORTAL)}`,
    });
    const run = await chatWidget(dir, 'stand', LOADER_OFF_PORTAL);
  ...
  it('адрес стенда, ведущий на портал заказчика, выкладку НЕ проходит', async () => {
    ...
    // (`b24-cbqwqo` и `b24-kbo5ls`, 140 вхождений в discovery/), и привязка к одному
  ...
  it('объявленное отсутствие и встраивание в выводе — расхождение и на стенде', async () => {
    ...
    const run = await chatWidget(dir, 'stand', CHAT_LOADER_NONE);
    expect(run.code, 'стенд принял встраивание при объявленном отсутствии адреса').toBe(1);
    expect(run.stderr).toMatch(/не ожидает встраивание чата/i);
  });
});
