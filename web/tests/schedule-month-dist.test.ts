...
describe('скрипт управления расписанием не выводит время', () => {
  ...
  // ─── Снятая проверка: «фильтрация опирается на признак записи» ──────────────
  // Здесь стояла третья проверка — `controlsScript()` обязан содержать
  // `dataset.months` или `data-months`. Она снята, а не починена, и причина не в
  // отсутствующем фильтре комментариев (соседняя проверка выше комментарии
  // отбрасывает, и его можно было добавить сюда одной строкой).
  //
  // Причина в направлении вывода: из НАЛИЧИЯ строки наличие фильтрации не следует.
  // Признак живёт и в мёртвом коде, и в комментарии, а `is:inline` доносит
  // комментарии до `dist` — то есть проверка утверждала поведение, а измеряла текст.
  // Ровно это направление в этой же работе запрещено мета-гейту
  // (`browser-test-gating.test.ts`, шапка про MONTH_MARKERS), и держать его здесь
  // было бы двойным стандартом.
  //
  // Измерено, а не предположено (голова `7b18284bd69fa1badb77b521713d78dc99722b81`):
  // при `const matchesMonth = true;` и признаке, оставленном только в комментарии,
  // этот файл давал 13 из 13 зелёных, включая снятую проверку, а браузерный прогон
  // `npm run test:e2e:schedule` — 7 падений из 16 (`@month-narrow`, `@month-city`,
  // `@month-empty-state`, `@month-pagination`, `@month-mobile`, `@month-whole-key`,
  // `@month-three-months`).
  //
  // Поведение закрыто там, где оно измеряется по ВЫДАЧЕ: `@month-narrow`,
  // `@month-whole-key`, `@month-three-months`. Они исполняются внутри гейта
  // публикации (`test:e2e:schedule` в job `e2e-smoke`), а их исполнение стережёт
  // мета-гейт — то есть снятие проверки не оставляет предмет без гейта.
});
