...
describe('демо-вывод: ни одной ссылки в CRM заказчика', () => {
  ...
  // (`form_links_match_mode`) утверждает то же самое, но он стережёт выкладку СТЕНДА и
  // в `Tests` не входит; этот файл — входит, поэтому утечка видна на PR, а не в момент
  // деплоя. Совпадение ответов двух проверок над одним предметом здесь намеренное:
  // AGENTS.md требует, чтобы такие проверки либо сходились, либо расхождение было
  // названо. Предметы у них разные (исходный вывод против выгружаемого каталога),
  // признак — один.
  //
  // Признак — ХОСТ назначения, а не форма пути. Перечисление путей (`crm_form`) уже
  // однажды сделало гейт деплоя слепым: `/news/` подписки в футере стоит на 267
  // страницах и в набор не попадал вовсе.
  const BITRIX_HOST = /^(?:https?:)?\/\/[^/?#]*\.bitrix24site\.ru(?:[/?#]|$)/;

  it('ни на одной странице нет ссылки на портал Bitrix24', () => {
    const offenders: string[] = [];
    for (const [path, html] of sources) {
      for (const m of html.matchAll(/href="([^"]*)"/g)) {
        if (BITRIX_HOST.test(m[1])) offenders.push(`${path} → ${m[1]}`);
      }
    }
    expect(
