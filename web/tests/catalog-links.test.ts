...
describe('связь семинара с группой курсов', () => {
  ...
  it('обновление каталога страхует связь и блокирует запись при разрыве', () => {
    ...
    // Блокировать обязана ЛЮБАЯ запись без итоговой группы, включая новую: первая
    // редакция смотрела на наличие прежней связи, и новый семинар без программы
    // сохранялся с адресом `/<slug>` вместо `/<группа>/<slug>`.
    expect(
      /if\s*\(!group\)\s*brokenLinks\.push/.test(code),
      'запись без группы не считается разрывом, если прежней связи не было (новый семинар)',
    ).toBe(true);
  });
});
