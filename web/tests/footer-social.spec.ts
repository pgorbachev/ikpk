 ...
 *    наблюдаются и проверяются сверкой ассета (хеш) и нормализованного тела `*Mark.astro`
 *    (`assertMarkFilesMatchRegistry` / `assertMarkAstroBodiesMatchAssets`; Requirement
 *    «Решение о применимости марки…», `design.md`, Решения 17 и 18);
 ...
 *
 * ПОЧЕМУ ПРОВЕРКИ БЫЛИ КРАСНЫМИ ДО РЕАЛИЗАЦИИ: подвал отдавал соцсети текстовыми ссылками
 * (`web/src/components/Footer.astro`, колонка «Подписывайтесь»), марок не было — не было
 * ни `<svg>`, ни доминирующей заливки. После реализации на `test/social-accounts` предмет
 * есть; этот заголовок — свидетельство красного прогона, не текущее состояние.
 *
...
test.describe('подвал: состав и подача внешних аккаунтов', () => {
  ...
  /**
   * Сценарий «марка сети показана»: маркой обязан быть каждый аккаунт, для которого реестр
   * применимости (`SOCIAL_MARKS_REGISTRY`) не называет исход `text-link`. Перечень
   * `MARK_ACCOUNTS` считается через `networksRequiringMark(...)` выше по файлу.
   */
