 ...
 *    показана» (частично: см. ДЕФЕКТ-1, ДЕФЕКТ-2 и ДЕФЕКТ-5 разбора);
 ...
 *
 * Спека говорит «в каждой теме, поддерживаемой сайтом», и правила получения перечня не
 * называет — это ДЕФЕКТ-7 разбора. Признак взят такой: тема по умолчанию плюс каждое
 * значение `data-theme`, для которого в `src/styles/tokens.css` переопределены токены.
  ...
  test('каждый аккаунт обозначен маркой, а не текстом', async ({ page }) => {
    ...
    expect(
      ...
      `подача аккаунта не марочная:\n${withoutMark.join('\n')}\n` +
        'Оговорка спеки про сеть, законно оставшуюся текстовой ссылкой, здесь не учтена: ' +
        'решение владельца нигде не объявлено машинно (ДЕФЕКТ-2 и ДЕФЕКТ-3 разбора).',
    ).toEqual([]);
  });

  /**
   * Запрет ЕДИНОЙ посторонней оправы. Объективно по выводу проверяется только остаток:
   * добавленное CSS-ом украшение (фон, рамка, скругление) между `<a>` и `<svg>`, одинаковое
   * у всех четырёх. Происхождение самой марки по выводу не устанавливается — это ДЕФЕКТ-5
   * разбора, и его закрывает сверка файла марки с первоисточником, а не эта проверка.
   */
...
test.describe('подвал: цвет марок', () => {
  /**
   * Доминирующая заливка марки в ЦЕЛЕВОМ РАЗМЕРЕ. Целевой размер в спеке не определён —
   * ДЕФЕКТ-6 разбора; здесь он взят как фактический размер отрисовки марки в подвале.
   *
  ...
  test('контраст доминирующей заливки к фону подвала не ниже 3:1 в каждой теме', async ({ page }, testInfo) => {
    ...
    expect(
      offenders,
      'контраст марки к фону подвала ниже 3:1 либо не измерен. Разрешённая правообладателем ' +
        'альтернативная версия марки нигде не объявлена машинно, поэтому исключение из порога ' +
        `эта проверка учесть не может (ДЕФЕКТ-3 разбора):\n${offenders.join('\n')}`,
    ).toEqual([]);
  });
  ...
  test('доминирующие заливки Youtube и Rutube различаются не менее чем на объявленный порог', async ({
    ...
    expect(
      ...
      `цветовое расстояние ΔE76 между марками ${distance.toFixed(1)} ниже объявленного порога ` +
        `${COLOR_DISTANCE_THRESHOLD}. Порог и метрика — решение исполнителя, спека их не называет ` +
        '(ДЕФЕКТ-4 разбора)',
