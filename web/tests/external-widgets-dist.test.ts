...
describe('секция отзывов подана так, как утверждено мокапом (вариант E)', () => {
  ...
  it('вводного абзаца о площадке в секции нет', () => {
    // Тот же README, `:984`, `Отдельно в этом же круге по просьбе владельца: убран`:
    // поясняющий абзац снят из мокапов по прямой просьбе владельца, и ни один
    // утверждённый кадр его не содержит. В выводе он вернулся в другой редакции.
    const section = reviewsSection();
