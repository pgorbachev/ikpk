...
describe('рейтинг и число отзывов существуют только внутри виджета', () => {
  ...
  it('признак ловит сводку с числами, ОТЛИЧНЫМИ от сегодняшних', () => {
    ...
    for (const markup of [
      ...
      // Формы, которыми сводку пишут на самом деле. Добавлены вместе с сужением признака:
      // сузить его, не расширив положительный корпус, значило бы проверить только то, что
      // он перестал краснеть, — а это одинаково верно и для исправного, и для отключённого.
      '<p>4,8 · 127 отзывов</p>',
      '<p>Рейтинг: 4,8</p>',
      '<p>Оценка 4,8 из 5</p>',
      '<p>Средняя оценка — 4,8</p>',
      '<p>127 отзывов на Яндекс.Картах</p>',
    ])
  ...
  it('признак не задевает телефоны, цены и даты', () => {
    ...
    for (const markup of [
      ...
      '<h2>Отзывы</h2>',
      // Настоящие названия учебных программ со стенда. «Оцен» здесь — диагностическая
      // оценка, а не рейтинг, и цифры рядом — номер ступени, код программы и цена.
      // Первая строка уронила обязательный прогон на main 24.09.2026 при неизменном коде:
      // на главной показывается БЛИЖАЙШИЙ семинар, поэтому состав страницы меняется сам,
      // от календаря. Вердикт гейта не должен зависеть от того, какой семинар ближе.
      '<p>LT-1: Интегративный подход к оценке. Уровень 1 · 60 000 ₽</p>',
      '<p>LT-1: Интегративный подход к оценке. Уровень 1</p><p>Институт Барраля</p><p>60 000 ₽</p>',
      // VM-5 отдельно: PR #258 сузил стем до конкретных словоформ и закрыл LT-1, но не эту
      // строку — «оценка» матчится ПОДСТРОКОЙ внутри «термооценка», левой границы регулярка
      // #258 не проверяет. Измерено на main@15a10bc9: ложное срабатывание сохранялось.
      '<p>VM-5: Мануальная термооценка и введение в висцеро-эмоциональную сферу</p><p>Уровень 5</p>',
    ])
