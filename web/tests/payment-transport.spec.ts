...
test.describe('3a.1a кнопка закрытия оформлена, а не UA-умолчание', () => {
  ...
  for (const theme of ['light', 'dark'] as const) {
    test(`кнопка закрытия оформлена и различима в теме ${theme}`, async ({ page }) => {
      ...
      // Признак ТОТ ЖЕ, что у проверки 3a.1 на строке 72 (`/закрыть|close/i`), а не более
      // строгий: два гейта над одним предметом обязаны отвечать одинаково. С `/закрыть/i`
      // имя «Close» прошло бы там и упало здесь — расхождение, о котором ни один из них
      // не сообщил бы (находка независимого ревью).
      expect(probe.name, 'у кнопки закрытия нет доступного имени «Закрыть»').toMatch(/закрыть|close/i);
      // Видимая метка есть. Отдельно от контраста: контраст сравнивает `color` с
      // `background`, и у ПУСТОЙ кнопки эти числа те же — то есть без этой проверки
      // потерянный глиф прошёл бы гейт незамеченным (нашла, перебирая способы сломать
      // контрол при зелёном гейте). Доступное имя приходит из `aria-label` и пустоту
      // глифа не выдаёт.
      expect(probe.glyph, 'у кнопки закрытия нет видимой метки — кружок пустой').not.toBe('');
