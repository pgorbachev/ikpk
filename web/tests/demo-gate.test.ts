...
function expandNpm(cmd: string, seen: ReadonlySet<string> = new Set()): string {
  return cmd.replace(/\bnpm\s+(?:run\s+)?([A-Za-z][\w:.-]*)/g, (whole, name: string) => {
    ...
    return `${whole} :: ${chain.map((n) => expandNpm(stripShellComments(webScripts[n]), next)).join(' && ')}`;
  });
}

/**
 * Полный текст ИСПОЛНЯЕМЫХ команд шага: `run` без shell-комментариев плюс раскрытые
 * npm-скрипты (тоже без комментариев).
 *
 * Комментарии снимаются ДО разбора, и это не косметика: все признаки ниже — «шаг собирает
 * сайт», «шаг запускает vitest с такой-то конфигурацией», «сборка стоит раньше проверок» —
 * читают этот текст. Пояснение вида `# npm run build:demo` без снятия комментариев делало бы
 * шаг шагом демо-сборки, а такое же пояснение в workflow вне гейта — «выносом демо-проверок
 * наружу». В обоих случаях гейт краснел бы от комментария, то есть от работы, которая ничего
 * не ломает; это ложное красное, зеркальное ложному зелёному, и стоит оно столько же —
 * доверия к цвету.
 */
function stepCommand(step: WorkflowStep): string {
  return step.run === undefined ? '' : expandNpm(stripShellComments(step.run));
}
...
/**
 * Комментарии вырезаются ДО извлечения литералов.
 *
 * Иначе `dist` или `dist-demo`, упомянутые в пояснении и взятые в обратные кавычки —
 * обычное дело в комментариях этого репозитория, — читаются как шаблонный литерал, и
 * файлу приписывается предмет, которого он не читает. Найдено на живом случае: комментарий
 * «стенд выкладывается из `web/dist`» в `demo-output.test.ts` дал этому файлу ВТОРОЙ
 * предмет, и проверка «у каждой проверки ровно один предмет» покраснела на файле, где
 * не изменилось ни одной строки кода. Это тот же дефект, от которого документация
 * `declaresDir` ниже уже защищалась вручную («сообщение вида „dist/ not found“»), — просто
 * закрытый не в том месте: не перечислением безобидных случаев, а признаком.
 *
 * Разбор посимвольный, потому что кавычки внутри комментариев и `//` внутри строк
 * (`'https://…'`) регуляркой не различаются: наивное вырезание `//…` до конца строки
 * съело бы половину URL в настоящих литералах.
 */
...
function literals(src: string): string[] {
  return [...stripComments(src).matchAll(/(['"`])((?:\\.|(?!\1)[^\\\n])*)\1/g)].map((m) => m[2]);
}
...
/**
 * Явный предел проверок этого файла. Дефолт основной конфигурации (20 000 мс) для них не
 * величина по существу, а ложное красное: каждая проверка здесь разбирает конфигурации vitest
 * и обходит ИМПОРТНОЕ ЗАМЫКАНИЕ выбранных ими файлов, то есть её длительность — это работа
 * трансформатора над десятками модулей, а не свойство предмета.
 *
 * Измерено на этой машине: самая долгая проверка («материал для проверки на месте») — 4,7 с
 * при средней загрузке 62 и 27,8 с при загрузке 98, когда рядом работали другие агенты. То
 * есть дефолт она перерастает не от дефекта, а от загрузки: тот же файл на тех же данных дал
 * `10 passed` и `1 failed` на двух одинаковых прогонах. Запас взят от наблюдённого худшего
 * времени с четырёхкратным множителем.
 *
 * Предел ничего не ослабляет: стерегомый дефект (демо-сборка или её проверки вне обязательного
 * прогона) определяется СОСТАВОМ разобранных workflow и конфигураций, а не временем; на успехе
 * проверка возвращается сразу. Ни одна ветвь этого файла не выводит отказ из того, что
 * что-то не уложилось во время.
 */
const GATE_TEST_TIMEOUT_MS = 120_000;

describe('демо-гейт: положение внутри обязательного прогона', { timeout: GATE_TEST_TIMEOUT_MS }, () => {
...
describe('демо-гейт: два вывода — два предмета', { timeout: GATE_TEST_TIMEOUT_MS }, () => {
...
describe('демо-гейт: предмета нет — «не выполнено»', { timeout: GATE_TEST_TIMEOUT_MS }, () => {
