...
describe('каталог статей: индексные инварианты', () => {
  ...
  it('достижимость статей для поискового обхода: недостижима ровно одна', () => {
    const inbound = new Set<string>();
    let templatesSeen = 0;
    let sitemapSkipped = 0;

    for (const p of allPages()) {
      // Потребитель этого графа — ПОИСКОВЫЙ ОБХОД, и карта сайта для него ребром
      // не является: на `/sitemap` стоит `content="noindex, nofollow"`, и
      // останавливает обход именно `nofollow`. Не `noindex` — тот лишь запрещает
      // индексировать саму страницу, ссылки на ней он не скрывает: человек без
      // JavaScript пользуется картой свободно, там обычные `<a>` на все 68 статей
      // без `rel`. Поэтому «недостижима одна статья» — утверждение о поисковом
      // обходе, а не о человеке; для человека без JS недостижимых нет.
