...
describe('каталог статей: индексные инварианты', () => {
  ...
  // Слабая форма намеренно: сильную («индексируемый путь по страницам списка
  // без JavaScript») система сегодня не даёт — см. KD-3 в спеке.
  it('у каждой страницы каталога есть входящая внутренняя ссылка', () => {
    ...
    expect(inbound.size, 'внутренних ссылок в сборке не найдено — проверять нечего').toBeGreaterThan(0);
    const orphans = catalogPages().filter((p) => !inbound.has(p));
    expect(
      orphans.slice(0, 8),
      `страниц каталога без входящей ссылки: ${orphans.length}\n${orphans.slice(0, 8).join('\n')}`,
    ).toEqual([]);
  });
});
