...
/**
 * Вариант со завершающим слэшем — каждому легаси-адресу, у которого его в карте нет.
 *
 * Без этого адрес вроде `/contacts/` обслуживается общим правилом нормализации из vhost
 * и получает ДВА путевых шага: 301 на `/contacts`, затем 301 на `/kontakty`. Спека
 * (`openspec/specs/static-serving/spec.md`) ограничивает цепочку двумя шагами и
 * закладывает первый шаг под канонизацию схемы и имени хоста — то есть второй путевой
 * шаг съедает весь бюджет, и появление http→https или www→канон сделает цепочку
 * трёхшаговой. На момент правки таких адресов было три: `/contacts`, `/promotions`,
 * `/sitemap.xml`; у остальных 254 вариант со слэшем в карте есть.
 *
 * Цель берётся та же, что у формы без слэша: правило ведёт сразу на канонический адрес,
 * а не на промежуточный.
 */
for (const [from, to] of [...seen]) {
  if (!from.endsWith('/') && !seen.has(`${from}/`)) seen.set(`${from}/`, to);
}
