...
/**
 * Пустое объявление — провал, а не успех: цикл сверки ниже идёт по этому списку, и на
 * пустом он не делает ни шага. Без guard'а скрипт напечатал бы «выполнены все 0
 * объявленных сценариев месяца» и вышел с нулём — вакуумность, выданная за отсутствие
 * дефектов. Guard тот же и по той же причине, что у сестринской сверки
 * (`tests/browser-test-gating.test.ts`, «объявленных меток месяца ноль — сверять
 * нечего»): потеря списка обязана краснеть в обеих точках, а не в одной.
 *
 * Симметрия ниже (`undeclared`) сюда не годится: она краснеет от метки `month-*` в
 * прогоне, а прогон без таких меток вместе с пустым объявлением остался бы зелёным.
 */
if (EXPECTED_MONTH_TAGS.length === 0) {
  fail(
    'объявленных меток месяца ноль — сверять нечего. Пустое объявление это «не смогла ' +
      'проверить», а не «дефектов нет»',
  );
}
