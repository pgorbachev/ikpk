...
export interface WorkerAudit {
  ...
  code: 'published' | 'rolled-back' | 'rollback-failed' | 'publication-failed' | 'checks-failed' | 'ci-failed' | 'provenance-changed' | 'active-unindexed' | 'main-changed';
...
const auditCodes = ['rollback-failed', 'publication-failed', 'checks-failed', 'ci-failed', 'provenance-changed', 'active-unindexed', 'main-changed'];
...
export function createWorkerAudit({ operation, error }: { operation?: unknown; error?: unknown }): WorkerAudit {
  ...
  const audit: WorkerAudit = { version: 1, status: operation === undefined ? 'refused' : 'success',
    code: operation === undefined ? (typeof source.code === 'string' && auditCodes.includes(source.code) ? source.code as WorkerAudit['code'] : 'publication-failed') : (typeof source.rollbackOfPublicationId === 'string' ? 'rolled-back' : 'published') };
  for (const [field, pattern] of Object.entries({ commit: /^[a-f0-9]{40}$/, snapshotId: /^snap:[a-f0-9]{64}$/,
    ...
    ...(audit.code === 'rolled-back' || operation === undefined ? { releaseId: /^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/ } : {}) })) {
  ...
  if (operation !== undefined) {
    const groups = object(audit.code === 'rolled-back' ? source.rollbackChecks : source.localChecks).groups;
