...
- `$openspec-continue-change` and `$openspec-new-change` may not be installed (core profile). When suggesting one that is unavailable, point to the CLI instead: `./bin/openspec status --change "<name>" --json` shows the next artifact and `./bin/openspec instructions <artifact-id> --change "<name>" --json` explains how to create it.
