...
for (const kind of ['cancelled', 'staged-without-prepare']) test(`REVIEW: ${kind} candidates cannot consume the four previous published release slots`, { timeout: 20000 }, async (t) => {
  ...
  for (let i = 0; i < 5; i++) {
    if (kind === 'cancelled') {
      ...
      // A finished upload does not imply prepare/activate: connection can end here.
      await transport.withLock((session) => session.stage({ releaseId: `abandoned-${i}`, sourceDir: source, expectedDigest: treeDigest }));
    }
  ...
  assert.deepEqual(retainedPublished, ['accepted-new', 'published-2', 'published-3', 'published-4', 'published-5'],
    'the five-release boundary promises current plus four preceding publications, not cancelled candidates');
});
