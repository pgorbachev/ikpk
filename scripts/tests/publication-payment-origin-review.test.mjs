...
import { readConfig } from '../../web/scripts/publication-context.ts';
const web = fileURLToPath(new URL('../../web', import.meta.url));

async function scenario(t, mismatch) {
  ...
  t.after(async () => { server.closeAllConnections(); await new Promise(resolve => server.close(resolve)); rmSync(temp, { recursive: true, force: true }); });
  ...
  catch { return { rejected: true, liveStatus: live.status }; }
  ...
  return { status, count, liveStatus: live.status, requests, actual, allowed };
}
