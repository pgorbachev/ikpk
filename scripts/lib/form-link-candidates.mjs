...
//
// Признак назначения: элемент, несущий CHAT_FACADE_ATTR, размечает СЕБЯ как носитель
// встраивания стороннего виджета — ни одно из его СОБСТВЕННЫХ атрибутов не может быть
// ссылкой формы, независимо от имени атрибута. Остальные значения возвращаются как есть:
// отбор по назначению (портал Bitrix24 / заглушка / crm_form) и сверка с ожиданием
// режима остаются в bash — здесь только извлечение и нормализация значения (trim, тот
// же приём, что раньше делал sed), деление труда не меняется.
//
...
for (const file of files) {
  ...
  for (const el of walk(doc)) {
    const isChatFacade = el.attrs.some((a) => a.name === CHAT_FACADE_ATTR);
    if (isChatFacade) continue;
