...
form_links_match_mode() {
  ...
  #
  # Извлечение РЕГИСТРОНЕЗАВИСИМО (`-i`), потому что имена атрибутов в HTML
  # регистронезависимы: `HREF="…"` — та же кликабельная ссылка, что и `href="…"`.
  # Найдено владельцем на 3604de4: без `-i` такая ссылка не попадала в набор вовсе, и
  # соседняя правильная заглушка делала гейт зелёным при живой утечке в CRM.
  #
  # ИЗВЕСТНОЕ ОГРАНИЧЕНИЕ, названное вслух: инструмент построчный, поэтому значение
  # атрибута, разорванное переводом строки (`href="\n  https://…"`), не извлекается.
  # Браузер по такой ссылке перейдёт. В сегодняшнем выводе таких значений нет
  # (проверено), закрытие требует разбора парсером — записано в docs/tech-debt.md.
  set +e
  all_links=$(grep -rohiE '(href|src|action)[[:space:]]*=[[:space:]]*("[^"]*"|'"'"'[^'"'"']*'"'"'|[^"'"'"'>[:space:]]+)' \
  ...
  #
  # Значение НОРМАЛИЗУЕТСЯ до сопоставления: снимается имя атрибута (по первому `=`, а
  # не по списку имён — иначе пришлось бы снова угадывать регистр, а `s///I` есть у GNU
  # sed и отсутствует у BSD), затем кавычки, затем пробелы по краям. Ведущий пробел
  # браузеру перейти по адресу не мешает, а заякоренный признак ломал — вторая половина
  # той же находки владельца.
  set +e
  form_links=$(printf '%s\n' "$all_links" \
    | sed -E 's/^[^=]*=[[:space:]]*//; s/^"//; s/"$//; s/^'"'"'//; s/'"'"'$//; s/^[[:space:]]+//; s/[[:space:]]+$//' \
