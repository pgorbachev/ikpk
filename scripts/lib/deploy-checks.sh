...
form_links_match_mode() {
  ...
  # Ссылка ловится в ЛЮБОМ атрибуте и при любом виде кавычек, а не только `href="…"`.
  # Находка ревью (F6): переписыватель `redirectFormLinksInDemo` принимает
  # `href\s*=\s*(?:"…"|'…')`, то есть одинарные кавычки автором уже допущены как
  # возможные в контенте, — а гейт их не допускал. Проверяющий, принимающий меньший
  # язык, чем переписыватель, оставляет щель ровно между ними. Здесь берутся все
  # значения атрибутов в кавычках плюс `src`/`action` без кавычек.
  set +e
  all_links=$(grep -rohE '(href|src|action)[[:space:]]*=[[:space:]]*("[^"]*"|'"'"'[^'"'"']*'"'"'|[^"'"'"'>[:space:]]+)' \
    "$dist" --include='*.html' 2>/dev/null)
  grep_rc=$?
  set -e
  if (( grep_rc > 1 )); then
  ...
  set +e
  form_links=$(printf '%s\n' "$all_links" \
    | sed -E 's/^(href|src|action)[[:space:]]*=[[:space:]]*//; s/^"//; s/"$//; s/^'"'"'//; s/'"'"'$//' \
    | grep -iE "$candidate_re" \
    | sort -u)
  grep_rc=$?
  set -e
  if (( grep_rc > 1 )); then
