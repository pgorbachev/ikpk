...
# --- BEGIN check_release_migrations ---
...
if [[ -n "${CMS_ARTIFACT_RELEASE:-}" && -n "${CMS_ARTIFACT_DIR:-}" ]]; then
  ...
  if [[ -d "$new_release" ]]; then
    ...
    # Состав миграций в релизе — отдельным скриптом, чтобы проверку можно было проверить.
    # Он приезжает на сервер тем же путём, что и сам bootstrap (весь файл уходит по ssh),
    # но живёт рядом: подробности и коды возврата — в его заголовке.
