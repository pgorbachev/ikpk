...
server {
  ...
  # 267 правил существовали только в репозитории — на сервере старые адреса
  ...
  # Нормализация завершающего слэша: у страницы ОДИН адрес.
  #
  # Канон сайта — адрес без слэша (web/astro.config.mjs, trailingSlash: 'never';
  # он же в rel="canonical" и в карте сайта). Форму со слэшем до сих пор сводили
  # к канону поимённые правила из include выше (сейчас их 267, из них 257 — про
  # форму со слэшем), но они порождаются из
  # discovery/url_map.csv — снимка СТАРОГО сайта. Маршрута, которого на старом
  # сайте не было, в карте нет, правила ему не достаётся, и try_files ниже
  # спокойно отдаёт 200 на обе формы: /video/33 и /video/33/ — два рабочих
  # адреса одной страницы. Замерено на стенде 29.08.2026; по свежей сборке так
  # вело себя 21 из 269 маршрутов.
  #
  # Правило общее, а не поимённое: список адресов отстаёт от предмета молча —
  # завтрашний маршрут снова окажется без правила.
  #
  # Регулярное выражение, а не `if` в location /: `if` внутри location
  # переигрывает выбор обработчика, а location — обычная фаза сопоставления.
  # Точное совпадение (`location = ...`) в nginx выигрывает у регулярного
  # выражения, поэтому все 267 правил include продолжают работать как раньше и
  # это правило у них ничего не отбирает. У каждого объявленного легаси-адреса
  # есть и вариант со слэшем, ведущий сразу на канон, — иначе такой адрес получал
  # бы ДВА путевых шага (301 на себя без слэша, затем 301 на канон) и съедал весь
  # двухшаговый бюджет цепочки, оставляя ноль под канонизацию схемы и хоста.
  #
  # `^(/.*[^/])/$` требует непустой путь перед слэшем, поэтому корень «/» под
  # него не подходит: https://example.com и https://example.com/ эквивалентны
  # (RFC 3986), и редирект там был бы петлёй.
  #
  # `$is_args$args` обязателен: `return` подставляет цель как есть и строку
  # запроса сам не переносит — без этого глубокая ссылка с параметром
  # (?section=3 в футере старого сайта) потеряла бы параметр.
  #
  # Своего Cache-Control блок не объявляет: ответ 301 наследует серверный,
  # как и правила перенаправления из include (набор add_header в nginx не
  # складывается по уровням — объявив свой, блок потерял бы серверный целиком).
  location ~ ^(/.*[^/])/\$ {
  ...
  location / {
    ...
    # а не через этот блок), живут и 267 правил перенаправления, подключённых
    # include выше.
