...
function parseArgs(): { baseUrl: string } {
  ...
  // Защита от ложного зелёного: до переключения DNS домен ikpk.su отвечает
  // СТАРЫМ сайтом, у которого все 767 редиректов и канониклы, естественно,
  // работают. Прогон против него показывает «всё ок», ничего не сказав о нашей
  // сборке. Пред-запусковая проверка обязана идти против staging/preview
  // (см. docs/migration-runbook.md, Pre-launch Checklist); против прод-домена
  // скрипт осмыслен только ПОСЛЕ переключения (post-launch мониторинг).
  if (/^https?:\/\/(www\.)?ikpk\.su$/i.test(normalized)) {
    console.warn(
      "\n⚠  ВНИМАНИЕ: цель — прод-домен ikpk.su.\n" +
        "   ДО переключения DNS он отвечает старым сайтом, и результат ничего\n" +
        "   не говорит о новой сборке (зелёный прогон = ложная уверенность).\n" +
        "   Для пред-запусковой проверки используйте staging/preview-домен.\n" +
        "   Осмысленно против прода — только после переключения.\n"
    );
  }
