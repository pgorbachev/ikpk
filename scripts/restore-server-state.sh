...
# путём. Восстановленный каталог — предмет решения оператора: подтверждённый откат на
# сохранённый релиз или новая публикация, и то и другое через launcher.
...
echo "Каталог ${target} восстановлен и НЕ активирован: раздача не тронута, переключение выполняет только launcher" >&2
